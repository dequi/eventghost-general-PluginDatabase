...
class Text:
    ...
    buttonInsertClipboard = "Insert clipboard"
    checkboxAsyncSpeaking = "Asynchonous speaking"
    normal = "Normal"
    slow = "Slow"
    fast = "Fast"
    silent = "Silent"
...
class TextToSpeech(eg.ActionClass):
    ...
    def __call__(self, voiceName, rate, voiceText, time, volume, async):
        ...
        def filterFunc(s):
            if s == "DATE":
                ...
                try:
                    ...
                    cliptxt = win32clipboard.GetClipboardData(win32con.CF_UNICODETEXT)
                finally:
        ...
        voiceObj.Speak("<context>" + voiceText + "</context>", async)
        print repr(voiceText)
        if time == 1:
            voiceObj.Speak(strftime("%X"), async)
        elif time == 2:
            voiceObj.Speak(strftime("%x"), async)
        elif time == 3:
            voiceObj.Speak(strftime("%x %X"), async)
       
    
    def GetLabel(self, voiceName, rate, voiceText, time, volume, async):
    ...
    def Configure(
        ...
        insertClipboardButton = wx.Button(panel, -1, text.buttonInsertClipboard)
        def OnButton(event):
            textCtrl.WriteText('{CLIPBOARD}')
            textCtrl.SetFocus()
        insertClipboardButton.Bind(wx.EVT_BUTTON, OnButton)
		
        asyncSpeakingCheckbox = wx.CheckBox(panel, -1, text.checkboxAsyncSpeaking)
        asyncSpeakingCheckbox.SetValue(async)
        ...
        sizer2 = eg.HBoxSizer(
            ...
            (insertClipboardButton, 0, wx.LEFT|wx.EXPAND|wx.ALIGN_LEFT, 5),
            (asyncSpeakingCheckbox, 0, wx.LEFT|wx.EXPAND|wx.ALIGN_LEFT, 5),
            ((5, 5), 1),
        )
        ...
        while panel.Affirmed():
            panel.SetResult(
                ...
                0,
                volumeCtrl.GetValue(),
				asyncSpeakingCheckbox.GetValue()
            )
