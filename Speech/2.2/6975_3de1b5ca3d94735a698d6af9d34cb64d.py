...
class Speaker(Thread):
    #ADDED: class variable to store on-going speaking TTS
    speakingTTS = None
    ...
    def __init__(
        ...
        suff,
        device,
        interruptSpeech = False #ADDED interrupt parameter
        ):
    ...
    def run(self):
        #ADDED: pauses existing tts instance
        if self.interruptSpeech and not Speaker.speakingTTS == None: #already speaking
            print 'Pause'
            Speaker.speakingTTS.Pause()
        ...
        Speaker.speakingTTS = tts #ADDED: saves tts instance in the speakingTTS class variable
        tts.Speak(self.text,0) 
        Speaker.speakingTTS = None
...
class Speech(eg.PluginClass):
    ...
    def __init__(self):
        ...
        self.AddAction(TextToSpeechInterrupt)
...
#ADDED: inheriated class from TextToSpeech that triggers TTS interruption
class TextToSpeechInterrupt(TextToSpeech):
    #text = Text
    ...
    def __call__(self, voiceName, rate, voiceText, suff, volume, device = None):
        ...
        sp=Speaker(
            ...
            suff,
            device,
            True #ADDED: To trigger interruption in Thread
        )
