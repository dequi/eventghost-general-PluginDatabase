...
def Setter(self, data, DevSerial, area):
    ...
    userid = str(self.plugin.userid)
    transport_url = str(self.plugin.transport_url)
    access_token = str(self.plugin.access_token)
    getreq = str("/v2/put/" + area + DevSerial)
...
class Nest(eg.PluginClass):
    ...
    class text:
        ...
        tbu = "Seconds Between Forced Log Updates :"
        authBox = "Basic Authentication"
        username = "Username:"
        password = "Password:"
        weather = "Weather Events:"
    class Device:
        def __init__(self, serial=None,structure=None,name=None,objects=[]):
            self.serial = serial
    ...
    def Configure(self, index=0, logging=False, loglocation="", tbu="", username="", password="", weather_events=False, serial=None):
    ...
    def __start__(self, index, logging, loglocation, tbu, username, password, weather_events):
        ...
        for Dev in self.Devices:
            ...
            for x in sorted(self.status["shared"][Dev.serial]):     #these values are thermostat specific  and are labeled  as DeviceName.xxx
                ...
                if self.logging and first == True:
                    ...
                    fo.write('\n') 
                    self.LastLogTime = time.time() + int(self.SecBeforeUpdate)
                if first != True:
