...
#       Redid Polling so that device status updates every 15 sec, Weather every 1 min, and gets energy update at 1am local time every 10 min (during that hour every 10 min to ensure retrieval)
#       Heat index and Wind chill are now rounded to curb excessive event generation
#   2014-05-16
#       New Fix for polling method
#   *2014-05-17
#       Timing fix again in polling method
#       minor code clean-up throughout 
#   *2014-05-22
#       Added Checker to help fix timeouts do to ssl connection type
#   *2016-11-21
#       Added Eco mode.
#   *2016-11-28
#       Fixed display schedule information
...
class Nest(eg.PluginClass):
    ...
    def Configure(self, index=0, logging=False, loglocation="", username="", password="", weather_events=False, tbu="", serial=None):
        ...
        if aborted:
            self.__start__(index,loggingCtrl.GetValue(), loglocationCtrl.GetValue(), usernameCtrl.GetValue(), passwordCtrl.GetValue(), weatherCtrl.GetValue(), tbuCtrl.GetValue())

    def __init__(self):
        self.AddAction(get_object)
        self.AddAction(set_heat_cool_mode)
        self.AddAction(set_eco_mode)
    ...
    def __start__(self, index, logging, loglocation, username, password, weather_events, tbu):
        ...
        self.Values_Retrival_Time = self.PluginStart - timedelta(seconds=61)
        self.LastFullLog = datetime.now() - timedelta(hours=2)
        self.abort = False
        self.Values_timestamp = 0
        self.Checker = threading.Thread(target = self.Checker, args = ())
        self.Checker.start()
    ...
    def Checker(self):
        #makes sure that timeouts do not cause the plugin to stop responding
        while True:
            if datetime.now() > self.Values_Retrival_Time + timedelta(seconds=60):
                self.abort = True
                time.sleep(5)
                self.abort = False
                self.poll = threading.Thread(target = self.Polling, args = ())
                self.poll.start()
            time.sleep(60) #waits an hour each interval
    ...
    def Polling(self):
        ...
        self.save_weather(True)
        for Dev in self.Devices:
            self.get_energy(Dev)
        eg.TriggerEvent("LoginSuccess", prefix="Nest")
        self.counterStatus = 0   #main device status retrieval   every 15 sec
        self.counterWeather = 0  #Weather counter                every 1 min
        self.counterEnergy = 0 #energy counter                   every 10 min
        while (self.abort == False):
            ...
            # get device status
            if FullCheck:
                time.sleep(30 - (self.counterStatus*2)) #so as to give last poll time to finish
                self.counterStatus = 0
                success = self.get_status()
                if success:
                    self.save_status(True)
            else:
                if self.counterStatus  > 29:
                    self.counterStatus = 0
                    success = self.get_status()
                    if success:
                        self.save_status()
            # get weather
            if FullCheck:
                self.counterWeather == 0
                success = self.get_weather()
                if success:
                    self.save_weather(True)
            else:
                if self.counterWeather > 119:
                    self.counterWeather = 0
                    success = self.get_weather()
                    if success:
                        self.save_weather()
            # energy report get will check for a complete hour (1am thru 2am) every 10 min
            if str(datetime.now().strftime("%H")) == "01":
                if self.counterEnergy > 1199:
                    self.counterEnergy = 0
                    for Dev in self.Devices:
                        self.get_energy(Dev)
            self.counterStatus = self.counterStatus + 1
            self.counterWeather = self.counterWeather + 1
            self.counterEnergy = self.counterEnergy + 1
            time.sleep(0.5)   #in seconds
        else:
            print "Ending Nest Capture"
    ...
    def get_status(self):
        ...
        try:
            #in try in case of connectivity errors
            transport = str(self.transport_url)
            transport = transport.replace('https://', '')
            transport = str(transport.replace(':443', ''))
            ...
            h = httpslib.HTTPSConnection(transport, 443, ssl_context=ctx)
            h.set_debuglevel(0)
            h.putrequest('GET', str('/v2/mobile/user.' + str(self.userid)))
            h.putheader('user-agent', 'Nest/2.1.3 CFNetwork/548.0.4')
    ...
    def get_energy(self, Dev):
        try:
            ...
            h = httpslib.HTTPSConnection(transport, 443, ssl_context=ctx)
            h.set_debuglevel(0)
            h.putrequest('POST', str('/v5/subscribe'))
            h.putheader('user-agent', 'Nest/2.1.3 CFNetwork/548.0.4')
            h.putheader('Authorization', str('Basic ' + str(self.access_token)))
            h.putheader('X-nl-user-id', str(self.userid))
            h.putheader('X-nl-protocol-version', '1')
    ...
    def save_weather(self, first=False):
        ...
        HIF = int(round(float(HI)))
        WCF = int(round(float(WC)))
        HIC = int(round(float(self.temp_in(HI))))
        WCC = int(round(float(self.temp_in(WC))))
        strwet = "heat_index_f"
        if self.logging and first == True:
            stringz =  "weather_current_" + str(strwet)
            fo.write(str(str(datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S ')) + stringz.ljust(65) + str(HIF)))
            fo.write('\n')
        if first != True:
            if getattr(self, "Values_weather_current_" + str(strwet)) != HIF:
                #writing to file if logging enabled
                if self.logging:
                    stringz =  "weather_current_" + str(strwet)
                    fo.write(str(str(datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S ')) + str(stringz.ljust(65)) + str(HIF)))
                    fo.write('\n')
                    self.logtype = "new"
               #if value has changed then issue an event
                if self.WeatherEvents:
                    self.Nest_Events(str(strwet),str(HIF), "weather_current")
        setattr(self, "Values_weather_current_" + str(strwet), HIF)
        self.CurrentWeather.append(str(strwet))
        strwet = "heat_index_c"
        if self.logging and first == True:
            stringz =  "weather_current_" + str(strwet)
            fo.write(str(str(datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S ')) + stringz.ljust(65) + str(HIC)))
            fo.write('\n')
        if first != True:
            if getattr(self, "Values_weather_current_" + str(strwet)) != HIC:
                #writing to file if logging enabled
                if self.logging:
                    stringz =  "weather_current_" + str(strwet)
                    fo.write(str(str(datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S ')) + str(stringz.ljust(65)) + str(HIC)))
                    fo.write('\n')
                    self.logtype = "new"
               #if value has changed then issue an event
                if self.WeatherEvents:
                    self.Nest_Events(str(strwet),str(HIC), "weather_current")
        setattr(self, "Values_weather_current_" + str(strwet), HIC)
        self.CurrentWeather.append(str(strwet))
        strwet = "wind_chill_f"
        if self.logging and first == True:
            stringz =  "weather_current_" + str(strwet)
            fo.write(str(str(datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S ')) + stringz.ljust(65) + str(WCF)))
            fo.write('\n')
        if first != True:
            if getattr(self, "Values_weather_current_" + str(strwet)) != WCF:
                #writing to file if logging enabled
                if self.logging:
                    stringz =  "weather_current_" + str(strwet)
                    fo.write(str(str(datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S ')) + str(stringz.ljust(65)) + str(WCF)))
                    fo.write('\n')
                    self.logtype = "new"
               #if value has changed then issue an event
                if self.WeatherEvents:
                    self.Nest_Events(str(strwet),str(WCF), "weather_current")
        setattr(self, "Values_weather_current_" + str(strwet), WCF)
        self.CurrentWeather.append(str(strwet))
        strwet = "wind_chill_c"
        if self.logging and first == True:
            stringz =  "weather_current_" + str(strwet)
            fo.write(str(str(datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S ')) + stringz.ljust(65) + str(WCC)))
            fo.write('\n')
        if first != True:
            if getattr(self, "Values_weather_current_" + str(strwet)) != WCC:
                #writing to file if logging enabled
                if self.logging:
                    stringz =  "weather_current_" + str(strwet)
                    fo.write(str(str(datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S ')) + str(stringz.ljust(65)) + str(WCC)))
                    fo.write('\n')
                    self.logtype = "new"
               #if value has changed then issue an event
                if self.WeatherEvents:
                    self.Nest_Events(str(strwet),str(WCC), "weather_current")
        setattr(self, "Values_weather_current_" + str(strwet), WCC)
        self.CurrentWeather.append(str(strwet))
        if self.logging:
                fo.close()
...
class set_heat_cool_mode(eg.ActionBase):
    ...
    class text:
        ...
        states = ["Off", "Heat", "Cool", "Range", "Eco"]
...
class set_eco_mode(eg.ActionBase):

    class text:
        name = "Set Eco Mode"
        description = "Set the Eco Mode for a Nest Thermostat.  Modes are: schedule (off), auto-eco (auto-away), manual-eco (on)."
        states = ["Schedule", "Manual-Eco"]
    ...
    def __call__(self, mode, DevSerial):
        ...
        if not str(mode).isdigit():
            ...
            try:
                ...
                print "Invalid Variable Data"
                return
        data = { 'eco': {'mode': mode,'mode_update_timestamp':int(time.time())}}
        data = json.dumps(data)
        Setter(self, data, DevSerial, "device.")

    def Configure(self, state="Schedule", serial=""):
        panel = eg.ConfigPanel()
        if self.plugin.SignedIN == "False":
            print "Not Signed in to Nest"
            panel.Destroy()
        ...
        for x in self.plugin.Devices:
            self.DevChoices.append(x.name)
        wx.StaticText(panel, label="Eco Mode: ", pos=(10, 10))
        statChoice = wx.Choice(panel, pos=(10, 30), choices=self.text.states)
        statChoice.Select(self.text.states.index(state))
        wx.StaticText(panel, label="Device: ", pos=(10, 60))
        statDevice = wx.Choice(panel, pos=(10, 80), choices=self.DevChoices)
...
class Schedule_Information(eg.ActionBase):      
    ...
    def __call__(self, day, DevSerial, pp = False):
        ...
        sl = []
        if day == "Sunday" or day == 0:
            key = "6"
        elif day == "Monday" or day == 1:
            key = "0"
...
class test_chart(eg.ActionBase):

    class text:
        name = "Test Chart from Chart Director"
