...
class SoundMixerWin32():
    ...
    def GetDeviceLines(self, deviceId=0):
        ...
        for i in range(mixercaps.cDestinations):
            ...
            # Add controls without source
            destinations.append((self.GetControls(hmixer, mixerline), None))

            # Loop over all sources
            for j in range(mixerline.cConnections):
                ...
                # Add controls with this source
                destinations.append((self.GetControls(hmixer, mixerline), source))

            # Add destinations
            result.append((destinations, destination))
    ...
    def GetControls(self, hmixer, mixerline):
        ...
        if numCtrls > 0:
            ...
            for i in range(numCtrls):
                ...
                result.append((mixerControl.dwControlID, mixerControl.szName.decode("mbcs"), controlClass["name"],controlClassTypeName))
...
class SoundMixerEx(eg.PluginClass):
    ...
    def GetTree(self, panel, classVisible, mixerSelect, controlSelect):
        ...
        # Get mixers
        mixers = self.mixer.GetMixerDevices()
        ...
        # Loop over ALL mixers
        for mixerId, mixerName in mixers:

            # Loop over ALL destinations for this mixer
            mixerItem = treeCtrl.AppendItem(rootItem, mixerName)
            destinations = self.mixer.GetDeviceLines(mixerId)
            for sources, destinationName in destinations:

                # Loop over ALL sources for this destination
                dstItem = treeCtrl.AppendItem(mixerItem, destinationName)
                for controls, sourceName in sources:

                    # Loop over ALL controls for this source
                    if sourceName is not None:
                        controlItem = treeCtrl.AppendItem(dstItem, sourceName)
                    else:
                        controlItem = dstItem
                    for controlId, controlName, controlClass, controlType in controls:

                        # Class not visible?
                        if not controlClass in classVisible:
                            continue

                        # Add item to tree
                        item = treeCtrl.AppendItem(controlItem, "%s (%s)" % (controlName, controlType))

                        # Setup description
                        if sourceName is not None:
                            description = "'%s - %s - %s'" % (destinationName, sourceName, controlName)
                        else:
                            description = "'%s - %s'" % (destinationName, controlName)
                        if multipleMixers:
                            description = "%s on '%s'" % (description, mixerName)

                        # Add id's and description for later use
                        treeCtrl.SetPyData(item, (mixerId, controlId, description))

                        # Highlight currently selected control
