...
eg.RegisterPlugin(
    name = "Speech with Audio Output",
    author = "MonsterMagnet",
    version = "1.0." + "$LastChangedRevision: 877 $".split()[1],
    description = (
...
class SpeechTest(eg.PluginClass):
    ...
    def __start__(self):
        ...
        # enumerate all available sound devices and store them in a dict with their
        # name as key
        soundcards = {}
        for soundcard in self.VoiceObj.GetAudioOutputs():
            soundcards[soundcard.GetDescription()] = soundcard
...
class TextToSpeech(eg.ActionClass):
    ...
    def __call__(self, voiceName, soundcardName, rate, voiceText, time, volume):
        ...
        try:
            voiceObj.AudioOutput = self.plugin.soundcards[soundcardName]
        except:
            self.PrintError (self.text.errorNoVoice % soundcardName)
        voiceObj.Rate = rate
        ...
        def filterFunc(s):
            if s == "DATE":
                ...
                return '</context><context ID="time">' + strftime("%I%M%p") + '</context><context>'
            else:
                return None
    ...
    def GetLabel(self, voiceName, soundcardName, rate, voiceText, time, volume):
    ...
    def Configure(
        ...
        try:
            VoiceObj = self.plugin.VoiceObj
            allSoundcards = self.plugin.soundcards.keys()
            allSoundcards.sort()
            try:
                soundcardIndex = allSoundcards.index(soundcardName)
            except:
                soundcardIndex = 0
        except:
            soundcardIndex = 0
            allSoundcards = []
        soundcardChoice = wx.Choice(panel, -1, choices=allSoundcards)
        soundcardChoice.Select(soundcardIndex)
        ...
        sizer3.AddMany(
            (
                ...
                (panel.StaticText(text.labelSound), 0, ACV),
                (soundcardChoice, 0, wx.EXPAND|wx.BOTTOM, 10),
        ...
        while panel.Affirmed():
            panel.SetResult(
                ...
                soundcardChoice.GetStringSelection(),
                rateCtrl.GetValue(), 
                textCtrl.GetValue(), 
                0,
                volumeCtrl.GetValue()
