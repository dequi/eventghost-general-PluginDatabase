...
#
#
# $LastChangedDate: 2009-11-24 17:34:46 +0100 (Di, 24 Nov 2009) $
...
eg.RegisterPlugin(
    name = "System",
    author = "Bitmonster",
    version = "1.0." + "$LastChangedRevision: 1238 $".split()[1],
    description = (
...
def isLowerThenVista():
    '''Return True if current OS is < then Windows Vista.'''
...
class System(eg.PluginBase):
    ...
    def __init__(self):
        ...
        if isLowerThenVista():
            group.AddAction(UnlockWorkstation)
...
class Playback(Thread):
    ...
    def run(self):
        sound = wx.Sound(self.wavfile)
        sound.Play(wx.SOUND_SYNC)
        eg.TriggerEvent(self.suffix, prefix = "System.Sound")
...
class PlaySound(eg.ActionWithStringParameter):
    ...
    class text:
        ...
        text3 = "Completion event suffix:"
        fileMask = "Wav-Files (*.WAV)|*.wav|All-Files (*.*)|*.*"
        
    
    def __call__(self, wavfile, wait = False, suffix = "PlaybackEnd"):
        if wait:
            self.sound = wx.Sound(wavfile)
            self.sound.Play(wx.SOUND_SYNC)
            eg.TriggerEvent(suffix, prefix = "System.Sound")
        else:
            playback = Playback(wavfile, "PlaybackEnd")
            playback.start()
    
    
    def Configure(self, wavfile='', wait = False, suffix = "PlaybackEnd"):
        panel = eg.ConfigPanel()
        text = self.text
        filepathCtrl = panel.FileBrowseButton(wavfile, fileMask=text.fileMask)
        waitCheckbox = panel.CheckBox(wait, text.text2)
        ...
        panel.sizer.Add(panel.StaticText(text.text3), 0, wx.EXPAND|wx.TOP, 10)
        panel.sizer.Add(suffixCtrl, 0, wx.EXPAND)
        panel.sizer.Add(waitCheckbox, 0, wx.EXPAND|wx.TOP|wx.BOTTOM, 10)
    
        while panel.Affirmed():
            panel.SetResult(filepathCtrl.GetValue(), waitCheckbox.IsChecked())
...
class UnlockWorkstation(eg.ActionBase):   
    name = "Unlock Workstation"
    description = \
        "Unlock Workstation"
    ...
    class text:
        user = "User name:"
        pasw = "Password:"
        userField = "User name field:"
        paswField = "Password field:"
        statBoxLabel = "Hot-keys of the login-screen"


    def __call__(self, credentials = None, u_key = "u", p_key = "p"):
        if credentials:
            args = [eg.MAIN_DIR + "\\MyUnlocker\\MyUnlockerLauncher.exe",]
            u, p = loads(b64decode(credentials))
            args.extend((u, p, u_key, p_key))
            Popen(args)

    def GetLabel(self, credentials):
        return "%s: %s" % (self.name, loads(b64decode(credentials))[0])


    def Configure(self, credentials = None, u_key = "u", p_key = "p"):
        panel = eg.ConfigPanel()
        text = self.text
        credentials = loads(b64decode(credentials)) if credentials else ("", "")
        userLabel = wx.StaticText(panel, -1, text.user)
        paswLabel = wx.StaticText(panel, -1, text.pasw)
        userLabel_2 = wx.StaticText(panel, -1, text.userField)
        paswLabel_2 = wx.StaticText(panel, -1, text.paswField)
        userCtrl = wx.TextCtrl(panel, -1, credentials[0])
        paswCtrl = wx.TextCtrl(panel, -1, credentials[1], style = wx.TE_PASSWORD)
        userCtrl_2 = wx.TextCtrl(panel, -1, u_key, size = (20, -1), style = wx.TE_CENTRE)
        paswCtrl_2 = wx.TextCtrl(panel, -1, p_key, size = (20, -1), style = wx.TE_CENTRE)
        bottomSizer = wx.StaticBoxSizer(
            wx.StaticBox(panel, -1, text.statBoxLabel),
            wx.HORIZONTAL
        )
        bottomSizer.Add(userLabel_2, 0, wx.TOP, 3)
        bottomSizer.Add(userCtrl_2, 0, wx.LEFT, 5)
        bottomSizer.Add((50, -1), 0)
        bottomSizer.Add(paswLabel_2, 0, wx.TOP, 3)
        bottomSizer.Add(paswCtrl_2, 0, wx.LEFT, 5)
        sizer = panel.sizer
        sizer.Add(userLabel)
        sizer.Add(userCtrl, 0, wx.EXPAND)
        sizer.Add(paswLabel, 0, wx.EXPAND|wx.TOP, 10)
        sizer.Add(paswCtrl, 0, wx.EXPAND)
        sizer.Add(bottomSizer, 0, wx.TOP, 20)

        while panel.Affirmed():
            user = userCtrl.GetValue()
            pasw = paswCtrl.GetValue()
            panel.SetResult(
                b64encode(dumps((user, pasw), 2)),
                userCtrl_2.GetValue(),
                paswCtrl_2.GetValue(),
            )
