# -*- coding: utf-8 -*-
#
# plugins/AIMP_2/__init__.py<?xml version="1.0" encoding="UTF-8" ?>
#
...
# Here we define a thread for listening to some changes of AIMP and trigger events
    def __start__(
        ...
        if events[0]==True or events[1]==True or events[2]==True:
            thread = Thread(
                target=self.Receive,
    ...
    def Receive(self, stopThreadEvent):
        while not stopThreadEvent.isSet():
            try:
                ...
                if self.events[1]==True:
                    tempObject=GetPosition()
                    newPlaylistPosition=unicode(tempObject())
                    tempObject=GetPlayingSongTitle()
                    newPlayingSongTitle=unicode(tempObject())
                    if self.oldPlaylistPosition!=newPlaylistPosition or self.oldPlayingSongTitle!=newPlayingSongTitle:
                        ...
                        self.TriggerEvent("PlayingTrack.Changed",[newPlaylistPosition,newPlayingSongTitle])
                if self.events[2]==True:
                ...
                if self.oldPlayerStatus!=0 and self.events[0]==True:
