...
import win32gui
import ctypes
from eg.WinApi.Dynamic import COPYDATASTRUCT, PCOPYDATASTRUCT,\
                              WM_COPYDATA, WM_USER
...
class AlbumPlayer(eg.PluginBase):
    ...
    def __stop__(self):
        if self.apHwnd:
            self.mr.RemoveWmUserHandler(self.SimpleMessageHandler)
            self.mr.RemoveHandler(WM_COPYDATA, self.XmlMessageHandler)
        if hasattr(self, 'mr'):
            if self.mr._ThreadWorker__thread.is_alive():
                wx.CallAfter(self.mr.Stop)
            self.mr = None
            del self.mr
    ...
    def XmlMessageHandler(self, hwnd, mesg, wParam, lParam):
        cpyData = ctypes.cast(lParam, PCOPYDATASTRUCT)
        msg = xmltodict.parse(ctypes.string_at(cpyData.contents.lpData))
    ...
    def SendCopydata(self, xmlMsg):
        ...
        return win32gui.SendMessage(self.apHwnd, WM_COPYDATA, self.mr.hwnd, cds)
...
class RemoteControl(eg.ActionBase):
    ...
    def __call__(self):
        if not self.plugin.apHwnd:
            return None
        return win32gui.SendMessage(self.plugin.apHwnd,
                         AP_REMOTE_CONTROL, self.value, 0)
...
class RemoteControlKey(eg.ActionBase):
    ...
    def __call__(self):
        if not self.plugin.apHwnd:
            return None
        return win32gui.SendMessage(self.plugin.apHwnd,
                         AP_REMOTE_CONTROL, AP_RC_RC_KEYS, self.value)
...
class ChangeVolume(eg.ActionBase):
    ...
    def Configure(self, volume = 50):
        ...
        spin = wx.SpinCtrl(panel, wx.ID_ANY,
                    style=wx.SP_ARROW_KEYS | wx.ALIGN_RIGHT | wx.SP_WRAP,
                    min=-100, max=100, initial=volume)
...
class Seek(eg.ActionBase):
    ...
    def __call__(self, position):
        if not self.plugin.apHwnd:
            return None
        return win32gui.SendMessage(self.plugin.apHwnd,
                         AP_REMOTE_CONTROL, self.value, position)
    ...
    def Configure(self, position = 0):
        ...
        spin = wx.SpinCtrl(panel, wx.ID_ANY,
                    style=wx.SP_ARROW_KEYS | wx.ALIGN_RIGHT | wx.SP_WRAP,
                    min=0, max=maxint, initial=position)
...
class SendSimpleXmlCommand(eg.ActionBase):
    ...
    '''
    def __call__(self):
        xmlMsg = r'<?xml version="1.0" encoding="utf-8"?>' \
                 r'<AlbumPlayerCommand id="%s"/>' % self.value
        return self.plugin.SendCopydata(xmlMsg)


ACTIONS = (
    (ConnectAP, 'Connect', 'Connect to AlbumPlayer', 'Connect to AlbumPlayer if it wasn\'t running at plugin start.', None),
    ...
    (eg.ActionGroup, 'AlbumPlayerRequests', 'Request info from AlbumPlayer', None, (
        ...
        (SendSimpleXmlCommand, "GetPlaylistNameList", "GetPlaylistNameList", "GetPlaylistNameList", 'GetPlaylistNameList'),
        (SendSimpleXmlCommand, "ClearPlaylist", "ClearPlaylist", "ClearPlaylist", 'ClearPlaylist'),
        (SendSimpleXmlCommand, "GetCollectionTypeList", "GetCollectionTypeList", "GetCollectionTypeList", 'GetCollectionTypeList'),
        ...
        (SendSimpleXmlCommand, "GetPartyModeSettings", "GetPartyModeSettings", "GetPartyModeSettings", 'GetPartyModeSettings'),
