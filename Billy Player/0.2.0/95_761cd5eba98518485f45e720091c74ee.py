...
eg.RegisterPlugin(
    ...
    description = (
        ...
        '<BR>Then You have to restart Billy plugin (Disable and then Enable). '
    ),
...
class Billy(eg.PluginClass):
        ...
        for tmpClassName, tmpName, tmpDescription, tmpHotKey in fnList:
            ...
            class tmpActionClass(eg.ActionClass):
                ...
                def __call__(self):
                    ...
                    HotKey = HotKeysArray[int(self.HotKey)]
                    if HotKeysArray[13]=="1": # Configuration file Billy.ini found !
                        if HotKey!=None:
                            if HotKey[0]=="1":
    ...
    def __start__(self, BillyPath=None):
        ...
        cp = SafeConfigParser()
        if os.access(os.path.dirname(BillyPath)+"//Billy.ini",os.F_OK):
            HotKeysArray[13]="1" # flag for tmpActionClass
            cp.read(os.path.dirname(BillyPath)+"//Billy.ini")
            for tmpClassName, tmpName, tmpDescription, tmpHotKey in fnList:
                try:
                    HotKeysArray[int(tmpHotKey)]=cp.get('Global hotkeys', tmpHotKey)
                except:
                    self.PrintError(self.text.text1+tmpName+self.text.text2)
        else:
            self.PrintError(self.text.text3)
