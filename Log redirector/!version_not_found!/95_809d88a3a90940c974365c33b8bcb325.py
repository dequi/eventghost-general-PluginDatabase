        ...
        while not self.abort:
            ...
            if self.check:
                ...
                if sz > self.maxSize*1048576:
                    eg.TriggerEvent(self.plugin.text.exceeded, prefix=self.plugin.text.prefix, payload = sz)
                    ...
                    eg.TriggerEvent(self.plugin.text.truncated, prefix=self.plugin.text.prefix, payload = path.getsize(fn))
...
class LogRedirector(eg.PluginBase):
    ...
    def Configure(
        ...
        def Validation(event = None):
            ...
            flg_4 = not val or not fltr or (fltr and eventsCtrl.GetItemCount()>0)
            flg = flg_1 and flg_2 and flg_3 and flg_4
