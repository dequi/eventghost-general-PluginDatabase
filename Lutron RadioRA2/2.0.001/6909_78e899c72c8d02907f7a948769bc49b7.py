...
timeclockEventDict = {  #These are the events for the timeclock events programmed into the system - these don't always match the index from the commands listed above - I have no idea why, but they will stay consistent once you identify them.
...
class LutronRadioRA2(eg.PluginBase):        
    ...
    def __init__(self):
        ...
        for devid, devtype in sorted(RADIORA2_CONFIG.items()):
            ...
            if devtype == 'Pico':              
                ...
                for className, descr, param, comp, pre in PICO_DEVICE_ACTIONS:
                    clsAttributes = dict(name=descr,parameterDescription=param,component=comp, prefix=pre, deviceid=devid, devicetype=devtype, devicelocation=devloc)   
                    cls = ClassType(devtype+className+devid, (KeypadAction,), clsAttributes)
                    group.AddAction(cls)
    ...
    def DoCommand(self, cmdstr):
        ...
        try:
            ...
            if (not self.isAuthenticated and (cmdstr==self.lutronuser or cmdstr==self.lutronpass)) or self.isAuthenticated: 
