...
# Revision history:
#
# 2010-08-17  Added a getter action to retrieve the Sun status with weather
#             data compensation
    ...
    def run(self):
        ...
        def LogToFile(s):
            ...
            if majorVersion > 5:
                ...
                fileHandle = open(progData+'/EventGhost/Log/Suntracker_'+self.name+'.html', 'a')
                fileHandle.write(logStr)
                fileHandle.close()
            else:
                if not os.path.exists('Log') and not os.path.isdir('Log'):
                    os.mkdir('Log')
                fileHandle = open('Log/Suntracker_'+self.name+'.html', 'a')
                fileHandle.write(logStr)
                fileHandle.close()


        def GetOffsetTimeSR(iO):
            if iO > 120:
                iO = 120
            if iO < -120:
                iO = -120
...
#            remain = 10
            ...
            if self.iWeather <> 0:
                ...
                if(
                    self.currCondition == "Cloudy"
                    or self.currCondition == "Fog"
                    or self.currCondition == "Rain"
                    or self.currCondition == "Light rain"
                    or self.currCondition == "Rain Showers"
                    or self.currCondition == "Freezing Rain"
                    or self.currCondition == "Drizzle"
                    or self.currCondition == "Freezing Drizzle"
                ):
                    trigTimeSR = GetOffsetTimeSR(
                                        - self.iWeather
                                        + self.iOffset
                                 )
                    trigTimeSS = GetOffsetTimeSS(
                                        - self.iWeather
                                        + self.iOffset
                                 )
                    self.timeCompensation = -self.iWeather + self.iOffset
                
                if(
                    self.currCondition == "Snow"
                    or self.currCondition == "Light snow"
                    or self.currCondition == "Mostly Cloudy"
                    or self.currCondition == "Partly Cloudy"
                    or self.currCondition == "Overcast"
                ):
                    trigTimeSR = GetOffsetTimeSR(
                                        - self.iWeather/2
                                        + self.iOffset
                    ...
                    self.timeCompensation = -self.iWeather/2 + self.iOffset
                ...
                if(
                    ...
                    or self.currCondition == "Undefined"
                ):
                    self.timeCompensation = self.iOffset
            ...
            if bMghost:
                LogToFile(self.text.lg_movingG_1 + self.name)
...
class Suntracker(eg.PluginClass):
    ...
    def LogToFile(self,s):
        ...
        if majorVersion > 5:
            ...
            if not os.path.exists(progData+"/EventGhost/Log") and not os.path.isdir(progData+"/EventGhost/Log"):
                os.makedirs(progData+"/EventGhost/Log")
            fileHandle = open(progData+'/EventGhost/Log/Suntracker.html', 'a')
            fileHandle.write(logStr)
            fileHandle.close()
        else:
            if not os.path.exists('Log') and not os.path.isdir('Log'):
                os.mkdir('Log')
            fileHandle = open('Log/Suntracker.html', 'a')
            fileHandle.write(logStr)
            fileHandle.close()
...
class GetSunStatusWeatherCompensated(eg.ActionClass):
    ...
    def __call__(
        ...
        # Set the conditions depending on the weather 
        self.trigTimeSR = self.GetOffsetTimeSR(0)
        self.trigTimeSS = self.GetOffsetTimeSS(0)
        ...
        if self.iWeather <> 0:
            ...
            if(
                ...
                self.trigTimeSR = self.GetOffsetTimeSR(- self.iWeather)
                self.trigTimeSS = self.GetOffsetTimeSS(- self.iWeather)
            ...
            if(
                ...
                self.trigTimeSR = self.GetOffsetTimeSR(- self.iWeather/2)
                self.trigTimeSS = self.GetOffsetTimeSS(- self.iWeather/2)
            ...
            if(
                ...
                if(
                    not self.plugin.summerSeasonBegins == "--"
                    and not self.plugin.summerSeasonEnds == "--"
                ):
                    if(
                        (
                            int(self.plugin.summerSeasonBegins) <=
                            int(self.plugin.summerSeasonEnds)
                        )
                        and
                        (
                            int(currMonth) >= int(self.plugin.summerSeasonBegins)
                            and int(currMonth) <= int(self.plugin.summerSeasonEnds)
                        )
                    ):
                        self.trigTimeSR = self.GetOffsetTimeSR(self.iWeather)
                        self.trigTimeSS = self.GetOffsetTimeSS(self.iWeather)
                    if(
                        ...
                        (
                            int(currMonth) >= int(self.plugin.summerSeasonBegins)
                            or int(currMonth) <= int(self.plugin.summerSeasonEnds)
                        )
                    ):
                        self.trigTimeSR = self.GetOffsetTimeSR(self.iWeather)
                        self.trigTimeSS = self.GetOffsetTimeSS(self.iWeather)
        ...
        if(
            self.trigTimeSS >= self.plugin.csSS
            or self.trigTimeSR < self.plugin.csSR
        ):
            self.sunIsUpW = False
        else:
            self.sunIsUpW = True
