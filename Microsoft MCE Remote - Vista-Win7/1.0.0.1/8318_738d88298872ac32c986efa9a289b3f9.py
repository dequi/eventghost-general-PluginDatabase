...
#{{Duc
def setCodeToEditCtrl( editCtrl, code ):
    if code is not None:
        try:
            editCtrl.SetValue(code if (code != "ERROR") else "")
            editCtrl.SelectAll() #Duc: make it easier to copy codes
            editCtrl.SetFocus()
        except:
            pass #Duc: avoid exceptions that the edit control can't be focused
#}}Duc
...
class TransmitIR(eg.ActionBase):
    ...
    def Configure(self, code='', repeatCount=0):
        ...
        editCtrl.SetMinSize((800, 200)) #Duc: make the dialog bigger so it's easier to see long codes
...
class IRLearnDialog(wx.Dialog):
    ...
    def __init__(self):
        ...
        #{{Duc
        self.editCtrl = wx.TextCtrl(self, style=wx.TE_MULTILINE)
        font = self.editCtrl.GetFont()
        font.SetFaceName("Courier New")
        self.editCtrl.SetFont(font)
        self.editCtrl.SetMinSize((800, 200)) #Duc: make the dialog bigger so it's easier to see long codes

        sizer.Add((5, 5))
        sizer.Add(self.editCtrl,0,wx.EXPAND)
    ...
    def OnClose(self, event):
        if (not hasattr(self, "exit")) or (self.exit != wx.ID_CANCEL): #Duc
            #Duc: fix bug: "exit" not defined when the X (close) button is clicked
            self.exit = wx.ID_OK if self.code else wx.ID_CANCEL #Duc
        event.Skip()
    ...
    def GotCode(self, freqs, code):
        ...
        self.exit = wx.ID_OK
        #self.Close() #Duc: This somehow makes EG crash
...
class MceMessageReceiver(object):
    ...
    def LearnIR(self):
        ...
        try: #Duc: Fixed exception: "exit" not defined when the X (close) button is clicked
