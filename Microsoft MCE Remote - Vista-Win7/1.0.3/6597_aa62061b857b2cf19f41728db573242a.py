...
class TransmitIR(eg.ActionBase):
    ...
    def Configure(self, code='', repeatCount=0, correctnessCount=1):
        ...
        def LearnIR(event):
            code = self.plugin.client.LearnIR(correctnessCtrl.GetValue())
            if not code is None:
...
class IRLearnDialog(wx.Dialog):
    ...
    def __init__(self, correctnessCount):
...
class MceMessageReceiver(object):
    ...
    def LearnIR(self, correctnessCount):
        ...
        self.freqs = [0]
        self.result = []
        self.learnDialog = IRLearnDialog(correctnessCount)
