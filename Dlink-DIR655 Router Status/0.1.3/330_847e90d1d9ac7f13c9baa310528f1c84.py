...
class Dir655Status(eg.PluginBase):
    ...
    def __start__(self, password, routerAddress, useHTTPS, eventFireSensitivity, stableSignalSeconds , updateRate ):
    ...
    def Configure(self, password="", routerAddress="192.168.0.1", useHTTPS=0, eventFireSensitivity = 15, stableSignalSeconds = 10 , updateRate = 2.0 ):
        text = self.text
        ...
        ctrlTexts = [ 'Password' , 'Router Address' , "Use HTTPS" , "Event fire sensitivity" , "Stable signal (sec)" , "Update rate (sec)" ]
        staticTexts = []
...
class RouterCommunicationThread(threading.Thread):
    ...
    def parse_wifi_assoc_reponse(self, response):
...
#            if ( dom.firstChild.nodeName != "wifi_assoc" ):
            ...
            try:
                ipElements = dom.getElementsByTagName('ip')
                for i in ipElements:
                    ip.append(i.firstChild.nodeValue)
                macElements = dom.getElementsByTagName('mac')
                for i in macElements:
                    mac.append(i.firstChild.nodeValue)
                modeElements = dom.getElementsByTagName('type')
                for i in modeElements:
                    mode.append(i.firstChild.nodeValue)
                rateElements = dom.getElementsByTagName('rate')
                for i in rateElements:
                    rate.append(i.firstChild.nodeValue)
                signalElements = dom.getElementsByTagName('rsi')
                for i in signalElements:
                    signal.append(i.firstChild.nodeValue)
    
                for i in mac:
                    if not connections.has_key( i ):
                        connections[ i ] = WifiConnection(
                            'amanda',
                            i,
                            ip[j] ,
                            mode[j] ,
                            float(rate[j].split('M')[0]) ,
                            int(signal[j]),
                            int('10')
                        )
        ...
        while not stopThreadEvent.isSet():
            ...
            if newFirmware:
                ...
                if resp.status == httplib.NOT_FOUND:
                    ...
                    return
            if not resp == None:
                self.parse_wifi_assoc_reponse( resp.read() );
