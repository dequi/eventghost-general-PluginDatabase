...
class Server(asyncore.dispatcher):
    ...
    def handle_close(self):
        self.plugin = plg
        if not plg.stopingplugin:
            eg.PrintError("Broadcaster: Socket Closed")
            if not plg.restarted:
                plg.restarted = True
                eg.PrintNotice("Broadcaster: Restarting")
                plg.__stop__()
                threading.Thread(
                                target=plg.__start__,
                                args=(
                                    plg.port,
                                    plg.payDelim,
                                    plg.zone,
                ...
                eg.PrintNotice("Broadcaster: Plugin has already restarted")
