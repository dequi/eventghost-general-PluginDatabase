...
eg.RegisterPlugin(
    name = "Harman Kardon",
    author = "Kingtd and Bitmonster",
    version = "1.0." + "$LastChangedRevision: 145 $".split()[1],
    kind = "external",
    description = "Allows control of a Harman Kardon AVR receiver through a serial port.",
)
...
class HK(eg.RawReceiverPlugin):
	...
	self.terminator = chr(0)+chr(242)
	    ...
	    ev2 = eventstring[st3+2:].strip()
            ...
            ev1 = ev1.encode('string_escape')
            ev2 = ev2.encode('string_escape')
	    if ev1.find("\\x00\\x00") == -1:
            	if ev1 != hktopmsg:
			hktopmsg=ev1
			if ev1 == "":
				self.TriggerEvent('Off','')
			else:
				self.TriggerEvent('Display Top',ev1)
                if ev2 != hkbotmsg and ev1 != "":
			hkbotmsg=ev2
			if ev2.find("VOL ") != -1:
			   self.TriggerEvent("Volume",ev2[4:])			
			self.TriggerEvent('Display Bottom',ev2)
