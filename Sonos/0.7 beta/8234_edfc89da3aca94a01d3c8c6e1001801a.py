            ...
            if data != '':
                ...
                if self.contentLength <= len(self.response['body']):
                    if self.connectionClose == 'close':
                        if globalDebug >= 2:
                            print "%s socket closing" % self.portused
                        self.close()
...
Need to fix: when a "stopped" zp is added to a "stopped" group, 
it triggers a "playing" event. Need to look at URI received and if
it's another ZP (coordinator) then I need to trigger based off its 
...
'''
need to add an event not be notified when grouptopology changes
how to handle name/ip changes vs actual grouping changes?

Need to fix: when a "stopped" zp is added to a "stopped" group, 
it triggers a "playing" event. This might need to be fixed above in the
transporteventrecieved functions. 
'''        
...
class WindowUUIDSelecttwo():   
    def __init__(self, uuid="", uuid2 = '', title="Select a Zone Player and item from the list below...", text1="", text2=""):
        ...
        while panel.Affirmed():
            ...
            panel.SetResult(
                FinalChoice.split("-")[1], #save only the uuid from the list.
                FinalPM.split("-")[1] #save only the uuid from the list.
            )
    ...
    def __init__(self, checkList=[], title="Select Zone Players from the list below..."):
        ...
        while panel.Affirmed():
            for box in ZPCheckBox:
                if box.GetValue():
                    if box.GetLabel().split("-")[1] not in selectedList: #only appended if not already added, need for testing command in config window
                        selectedList.append(box.GetLabel().split("-")[1]) #save only the uuid from the label and only the ZPs checked.
                else:
                    try:
                        selectedList.remove(box.GetLabel().split("-")[1]) #removes from list if added, this is needed when testing command in config window
                    except:
                        pass
            panel.SetResult(
                selectedList
...
class SetSleepTimer(eg.ActionBase):
    ...
    def __call__(self, uuid="", timer="30"):
        ...
        if uuid not in globalZPList:
            print "!!! zone player no longer in globalZPList (not found on network) !!!"
            return
        globalZPList[uuid].SendConfigureSleepTimer(timer)
        if globalDebug >= 1:
...
'''
to group you just set SetAVTransportURI to Zp that is coordinator:  x-rincon:RINCON_000E58A4D8E001400
will create a config window that allows for check boxes. You will select all ZPs you want in the group.
then the action will look at the current ZPs and find a curent coordinator that is playing music. Then group all ZPs to 
that coordinator. If no zp is playing music, then just have last coordinator in loop become the coordinator of the new group. 
'''     
            ...
            for (box, vol, mute) in zip(ZPCheckBox, volRange, muteCB):
                if box.GetValue():
                    ...
                    try: #if part of dict, remove ZP
                        del selectedList[box.GetLabel().split("-")[1]]
                    except:
                        pass
            panel.SetResult(
                selectedList
...
class NotificationRestore(eg.ActionBase):        
    ...
    def __call__(self, restoreDelay=10):
        ...
        eg.scheduler.AddTask(restoreDelay, self.RestoreTrackStateNow)
...
class PlayLineIn(eg.ActionBase):
    ...
    def Configure(self, uuid="", linein=""):
        ...
        text2 = '''NOTE: SONOS will accept this command all the time, \nbut if the ZP does not have a Line-In or nothing \nis plugged into it, SONOS will clear the \nLine-In from the currently playing track '''
...
class StartPlayList(eg.ActionBase):
    ...
    def Configure(self, uuid="", track="", dscr="", uri="", urimetadata="", upnpclass="", isplaylist=""):
        try:
            ...
            print "ERROR StartPlayList: something went wrong %s" % e
