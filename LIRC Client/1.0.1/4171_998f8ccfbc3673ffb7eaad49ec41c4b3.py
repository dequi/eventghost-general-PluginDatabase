...
based on data received from the LIRC-server.  This code is an adaptation
of the work originally done by Jinxdone
...
class Text:
    version = version
    title = "LIRC Client plugin v" + version + " by Chris Strutton"
    host = "Host:"
    port = "Port:"
...
##           self.ReconnectTimer()
    ...
    def __stop__(self):
        if self.Chat:
            self.Chat.gracefulclose = 1
            self.Chat.handle_close()
        self.Chat = None

    def InitConnection(self):
        self.Chat = LircChat.LircChat(self.host, self.port,ButtonCallBack = self.FireEvent)
        while not self.Chat.Connected:
            eg.Print ("Waiting for Chat.Connected")
            asyncore.poll()
...
##           self.ReconnectTimer()
    ...
    def FireEvent(self):
        ...
        if self.Chat.GetCodeCount():
            code = self.Chat.GetNextCode()
        if self.addremote:
            EgEvent = code[3] + ":" + code[2]
        else:
            EgEvent = code[2]
...
# The workhorse method.. (how tiny)!
        def __call__(self, msg):
            eg.Print("call: ",msg)
            self.plugin.Chat.SendDirective(directive = msg)
##            while self.plugin.Chat.BusyFlag:
##                asyncore.loop(count=1)

        def UpdateRemoteList(self):
            eg.Print("Update Remote List")
            self.remotes = self.plugin.Chat.GetList()
            eg.Print(self.remotes)
            self.remotelist = []
            for remote in self.remotes:
                self.remotelist.append(remote[0])
                eg.Print(self.remotelist[-1])
        ...
        def Configure(self, actionStr=""):
            ...
            def UpdateActionText():
                ...
                newstr = "%s %s %s" % (cmdlist[cmdCtrl.GetSelection()],
            ...
            actionCtrl = wx.Choice(panel, -1, choices=remotes[0][1])  ##[1]
            actionCtrl.SetSelection(0)
...
##            eg.Print(self.event, " ", str(len(self.event)))
##            i += 1
##            if skipit == 1:
##               if self.event == "END":
##                  skipit = 0
