...
class MyTimer():
	...
	def __init__(self, time, callabck):
...
class AtiRemoteWonder2(eg.PluginBase):
	...
	def __start__(self, mousemaxspeed=10, mouseacceleration=1, debugging=False, mousereset=False, mousetimeout=0.25, mouseresolution=1.0):
		...
		self.mousemaxspeed = mousemaxspeed / mouseresolution
		self.mouseacceleration = mouseacceleration / mouseresolution
	...
	def Callback1(self, (device, x, y)):
		...
		if self.debugging:
			res = (
				...
				'direction: '+str(dict(x=x, y=y, angle=degee))[1:-1],
	...
	def Configure(self,
		...
		resolCtrl = panel.SpinIntCtrl(mouseresolution, min=1)
