...
#        self.AddAction(StopRepeating)
    ...
    def Configure(self, ip="127.0.0.1", port="80", EnableEvents=False, EventWorkaround=False, EventLog=False):
#    def Configure(self, ip="127.0.0.1", IPs = ['127.0.0.1', '192.168.0.100']):
        ...
        EnableEventsCheckBox = wx.CheckBox(panel, -1, 'Enable events.')
        CheckBox = wx.CheckBox(panel, -1, 'Event workaround.')
        CheckBox2 = wx.CheckBox(panel, -1, 'Log raw event from XBMC.')
        EnableEventsCheckBox.SetValue(EnableEvents)
        CheckBox.SetValue(EventWorkaround)
        CheckBox2.SetValue(EventLog)
...
#        panel.sizer.Add(textControl, 1, wx.EXPAND)
        ...
        panel.sizer.Add(textControl2)
        panel.sizer.Add(EnableEventsCheckBox)
        panel.sizer.Add(CheckBox)
        panel.sizer.Add(CheckBox2)
        while panel.Affirmed():
            panel.SetResult(textControl.GetValue(), textControl2.GetValue(), EnableEventsCheckBox.GetValue(), CheckBox.GetValue(), CheckBox2.GetValue())

    def __start__(self, ip='127.0.0.1', port='80', EnableEvents=False, EventWorkaround=False, EventLog=False):
        self.ip = ip
        self.port = port
        self.EnableEvents = EnableEvents
        self.eventworkaround = EventWorkaround
...
'13099':'WINDOW_PYTHON_END',
			...
			def get():
				try:
					...
					try:
						return s.recvfrom(16384)
					except socket.error:
						import sys
						eg.PrintError('socket.error: ' + sys.exc_info())
					except:
						import sys
						eg.PrintError('Error2: ' + sys.exc_info())
				except:
					import sys
					print 
					eg.PrintError('Error: ' + sys.exc_info())
			...
			while not stopThreadEvent.isSet():
				...
				#print "Got data: %s" % shlex.split(repr(message))
