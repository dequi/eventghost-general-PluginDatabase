...
#====================================================================
    ...
    def __call__(
        ...
        items=len(choices)
        #font = self.plugin.menuDlg.GetFont()
        #font.SetPointSize(32)
        #if bold:
        #    font.SetWeight(wx.BOLD)
        font = wx.FontFromNativeInfoString(fontInfo)
        self.plugin.menuDlg.SetFont(font)
        # menu height calculation:
        h=self.plugin.menuDlg.GetCharHeight()
        height0 = items*h+5
        ...
        for item in ExtractFromList(choices,0):
            width_lst.append(self.plugin.menuDlg.GetTextExtent(item+' ')[0])
        width = max(width_lst)
    ...
    #    boldChkBoxCtrl.Bind(wx.EVT_CHECKBOX, OnBoldSwitch)
            ...
            if label.strip()<>"":
                if os.path.isfile(filepath):
                    if ExtractFromList(self.choices,0).count(label)==1:
                        if ExtractFromList(self.choices,1).count(filepath)==1:
                            self.oldSel=sel
        ...
        def OnTextChange(evt):
            if self.choices<>[]:
                ...
                label = labelCtrl.GetValue()
                filepath = filepathCtrl.GetValue()
                self.choices[sel]=(label,filepath) #ERR
                listBoxCtrl.Set(ExtractFromList(self.choices,0))
                listBoxCtrl.SetSelection(sel)
                if label.strip()<>"":
                    if os.path.isfile(filepath):
