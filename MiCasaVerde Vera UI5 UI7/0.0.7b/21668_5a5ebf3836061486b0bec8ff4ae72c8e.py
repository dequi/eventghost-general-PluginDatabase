...
#
#    -- Security Sensors
#       luup.variable_watch("EventGhostReporting","urn:micasaverde-com:serviceId:SecuritySensor","Armed",nil)
#       luup.variable_watch("EventGhostReporting","urn:micasaverde-com:serviceId:SecuritySensor","Tripped",nil)
#
...
#
#    -- Light Sensors
#       luup.variable_watch("EventGhostReporting","urn:schemas-micasaverde-com:service:LightSensor:1","CurrentLevel",nil)
#
#    -- Power Metering
...
eg.RegisterPlugin(
    ...
    version = "0.0." + "$LastChangedRevision: 7b $".split(' ')[1],
    canMultiLoad = True,
    kind = "other",
...
class ServerHandler(asynchat.async_chat):
    ...
    def SendEvent(self,sendPayload):
        ...
        if serviceName[3] == 'SwitchPower1':
            kwargs['suffix'] = 'Switch.DeviceID-'+devID+'.'

            kwargs['suffix'] += 'Off' if newState == '0' \
                                    else 'On'

        elif serviceName[3] == 'Dimming1':
            kwargs['suffix'] = 'Dimmer.DeviceID-'+devID+'.DimLevel-'+newState

        elif serviceName[3] == 'SecuritySensor1':
            kwargs['suffix'] = 'Security.DeviceID-'+devID+'.'
            ...
            kwargs['suffix'] = 'HVAC.DeviceID-'+devID+'.Mode-'+newState

        elif serviceName[3] == 'HVAC_FanOperatingMode1':
            kwargs['suffix'] = 'HVAC.DeviceID-'+devID+'.FanMode-'+newState

        elif serviceName[3] == 'TemperatureSensor1':
            kwargs['suffix'] = 'Temperature.DeviceID-'+devID+'.Current-'+newState

        elif serviceName[3] == 'TemperatureSetpoint1_Heat':
            kwargs['suffix'] = 'HVAC.DeviceID-'+devID+'.HeatSetTemperature-'+newState

        elif serviceName[3] == 'TemperatureSetpoint1_Cool':
            kwargs['suffix'] = 'HVAC.DeviceID-'+devID+'.CoolSetTemperature-'+newState

        elif serviceName[3] == 'HumiditySensor1':
            kwargs['suffix'] = 'Humidity.DeviceID-'+devID+'.Current-'+newState

        elif serviceName[3] == 'DoorLock1':
            kwargs['suffix'] = 'Doorlock.DeviceID-'+devID+'.'

            kwargs['suffix'] += 'Unlocked' if newState == '0' \
                                    else 'Locked'

        elif serviceName[3] == 'WindowCovering1':
            kwargs['suffix'] = 'WindowCovering.DeviceID-'+devID+'.'

            kwargs['suffix'] += 'Closed' if newState == '0' \
                                    else 'Open'

        elif serviceName[3] == 'LightSensor':
            kwargs['suffix'] = 'LightSensor.DeviceID-'+devID+'.'

            kwargs['suffix'] += 'Off' if newState == '0' \
                                    else 'On'

        elif serviceName[3] == 'EnergyMetering1':
            kwargs['suffix'] = 'Energy.DeviceID-'+devID+'.WattsUsed-'+newState

        else:
            kwargs['suffix'] = 'NewDevice.'+devID
