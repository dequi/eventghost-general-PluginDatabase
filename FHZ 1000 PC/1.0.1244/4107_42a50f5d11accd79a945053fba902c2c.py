...
eg.RegisterPlugin(
    ...
    kind="external",
    author="Bitmonster",
    version = "1.0." + "$LastChangedRevision: 1244 $".split()[1],    
)
...
def intToQuad(number):
  res = []
  curRem = number%4
  res.append(str(curRem+1))
  number /= 4
  curRem = number%4
  res.append(str(curRem+1))
...
class Fhz1000Pc(eg.PluginBase):
    ...
    def HandleReceive(self, serial):
        ...
        sensorMessage = [chr(0x81),chr(0x0b),chr(0x04)]
        data = serial.Read(512)
        #print "HR: " + " ".join(["%02X" % ord(c) for c in data])
        
        # sensorMessages kommen in bis zu zwei separaten Events.
        # Daher wird in der globalen Variable fhzCommand über beide Events hinweg der String zusammengesetzt.              
        # Die Eventnummer (0 oder 1) wird in fhzEventCounter festgehalten.
        
        # Schon das 2. Event?
        if fhzEventCounter == 1:
            # Ja, hintendran hängen
            fhzCommand += data
            # Zähler zurücksetzen
            fhzEventCounter = 1 - fhzEventCounter
        else:
            if (len (data) < 3):
                # Manchmal besteht der erste Teil der Nachricht nur aus 2 Byte, dann muss in jedem Fall
                # die 2. Nachricht auch noch gelesen werden.
                fhzCommand = data
                fhzEventCounter = 1 - fhzEventCounter                
            else:
                if (data[0] == sensorMessage[0]) & (data[1] == sensorMessage[1]) & (data[2] == sensorMessage[2]):
                    # Handelt es sich um eine sensorMessage?
                    # Ja! Nachricht speichern...
                    # print "sensorMessage erkannt!"
                    fhzCommand = data
                    # ... und Eventzähler auf 1 setzen.
                    fhzEventCounter = 1 - fhzEventCounter
                else:
                       # Nein, einfach so ausgeben.
                    if eg.debugLevel:
                        print "HR: " + " ".join(["%02X" % ord(c) for c in data])

        if (len (fhzCommand) == 13):
            fhzEventCounter = 0
            hc1 = intToQuad(ord(fhzCommand[8]) / 16)
            hc2 = intToQuad(ord(fhzCommand[8]) % 16)
            hc3 = intToQuad(ord(fhzCommand[9]) / 16)
            hc4 = intToQuad(ord(fhzCommand[9]) % 16)
            hc5 = intToQuad(ord(fhzCommand[10]) / 16)
            hc6 = intToQuad(ord(fhzCommand[10]) % 16)
            hc7 = "%02X" % ord(fhzCommand[12])
            # Event auslösen
            eg.TriggerEvent("Fernbedienung-" + hc1 + hc2 + "-" + hc3 + hc4 + "-" + hc5 + hc6 + "-" + hc7)
            #print "HR: " + " ".join(["%02X" % ord(c) for c in fhzCommand])
        else:       
            if eg.debugLevel:		
                print "HR: " + " ".join(["%02X" % ord(c) for c in fhzCommand])
