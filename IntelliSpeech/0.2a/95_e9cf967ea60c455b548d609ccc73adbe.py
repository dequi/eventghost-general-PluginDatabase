...
# -------------------------------------------
# 0.2a by Pako 2014-11-10 08:23 UTC+1
#       - bugfix
...
GEAR10 = (
        ...
        "gBYtDAEJloK6ezgJDPRFtLgNoSt3DAFiCezZw2m4DvDuB5UsDgKBAOlSEFhJ4mCYdoft"
...
class Track(object):
    ...
    def __init__(self, plugin, txt, src, vol, ix):
        self.plugin = plugin
        self.ix = ix
        resp = plugin.Request(
            u"http://translate.google.com/translate_tts",
            data = {"tl":src, "q":txt.encode("utf-8")}
        )
        if resp.ok and resp.headers['content-type'] == 'audio/mpeg' and\
            int(resp.headers['content-length']) == len(resp.content):
                if plugin.stopTTS:
                    return
                retCode, data = plugin.popen(resp.content, vol)
                if retCode:
        ...
        try: # debug
            ...
            for item in lst:
                if len(item) > 100:
                    ...
                    txtList.append(item)
            if txtList:
                dd = {0:Track(self.plugin, txtList[0], src, vol, ix)}            
                lnLst = len(txtList)
                for i in range(lnLst):
                    if self.plugin.stopTTS:
                        break
                    if i < lnLst - 1 :
                        dd[i+1] = Track(self.plugin, txtList[i+1], src, vol, ix)
                    if dd[i].track:
                        dd[i].play()
                    del dd[i]
...
#        staticBoxSizer.Add(mp3Ctrl, 0, wx.TOP|wx.EXPAND, 5)
    ...
    def Detect(self, txt):
        ...
        resp = self.Request(url, data = body)
        src = None
        if resp.ok:
            answ=resp.content
            try:
...
class SpeechToText(eg.ActionBase):
    ...
    def Configure(
        ...
        ctrl6.SetToolTipString(text.toolTip)
