...
class ShowMessageBox(eg.ActionBase):
    ...
    def showMessageBox(self,title,body,alias,payload,options):
            ...
            if result==1:
                ...
                result=str(result)
            eg.TriggerEvent("MessageBox."+alias+"."+result, payload)
            return result
    
    def __call__(self, title, body="", alias="",payload="", blocking=False, options=0):
        if blocking:
    ...
    def Configure(self, title="", body="", alias="", payload="", blocking=False, options=0):
        
        def getOptions():
            result=0
            result+=buttonsArr[buttonsCtrl.GetSelection()]
            result+=iconArr[iconCtrl.GetSelection()]
            result+=optionsArr[optionCtrl.GetSelection()]
            return result
        ...
        if options>=MB_TOPMOST:
            ...
            optionid=3
            options-=MB_SYSTEMMODAL
        iconid=0
        if options>=MB_ICONINFORMATION:
            iconid=1
            options-=MB_ICONINFORMATION
        elif options>=MB_ICONWARNING:
            iconid=3
            options-=MB_ICONWARNING
        elif options>=MB_ICONQUESTION:
            iconid=2
            options-=MB_ICONQUESTION
        elif options>=MB_ICONERROR:
            iconid=4
            options-=MB_ICONERROR
        buttonid=0
        if options>=MB_RETRYCANCEL:
            buttonid=2
            options-=MB_RETRYCANCEL
        elif options>=MB_YESNO:
            buttonid=4
            options-=MB_YESNO
        elif options>=MB_YESNOCANCEL:
            buttonid=5
            options-=MB_YESNOCANCEL
        elif options>=MB_ABORTRETRYIGNORE:
            buttonid=3
            options-=MB_ABORTRETRYIGNORE
        ...
        box0 = panel.BoxedGroup(text.main, (st2, titleCtrl), (st3, bodyCtrl), (st1, aliasCtrl), (st4, payloadCtrl))
        ...
        optionCtrl.SetSelection(optionid)
        panel.sizer.AddMany(((box0, 0, wx.EXPAND), (buttonsCtrl, 0, wx.EXPAND), (iconCtrl, 0, wx.EXPAND), (optionCtrl, 0, wx.EXPAND)))
        panel.AddLine(waitCtrl)
        while panel.Affirmed():
            panel.SetResult(
                titleCtrl.GetValue(), 
                bodyCtrl.GetValue(), 
