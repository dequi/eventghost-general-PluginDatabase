...
class MessageBoxDialog(wx.Dialog):
    ...
    def __init__(
        ...
        topSizer.Add(title,0,wx.TOP,5)
        topSizer.Add((1,1),1,wx.EXPAND)
        topSizer.Add(icon2,0,wx.LEFT|wx.RIGHT,10)
        mainSizer = wx.BoxSizer(wx.VERTICAL)
        mainSizer.Add(topSizer, 0, wx.EXPAND|wx.TOP|wx.BOTTOM, 10)
        messSizer = wx.BoxSizer(wx.HORIZONTAL)
        messSizer.Add((-1,-1), 1, wx.EXPAND)
        messSizer.Add(message, 0, wx.EXPAND|wx.LEFT|wx.RIGHT, 10)
        messSizer.Add((-1,-1), 1, wx.EXPAND)
        mainSizer.Add(messSizer, 0, wx.EXPAND|wx.ALIGN_CENTER_HORIZONTAL)
...
class ShowMessageBox(eg.ActionBase):
    ...
    def showMessageBox(self,title,body,alias,payload,options):
            optionid, iconid, buttonid = self.getFlags(options)
            if not alias:
                alias=title
            hwnd = eg.document.frame.GetHandle() if optionid == 1 else 0
            result = MessageBox(hwnd, body, title, options)
