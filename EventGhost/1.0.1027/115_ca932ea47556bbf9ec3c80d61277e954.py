...
#
#
# $LastChangedDate: 2009-06-01 17:31:15 +0200 (Mo, 01 Jun 2009) $
...
eg.RegisterPlugin(
    ...
    kind = "core",
    version = "1.0." + "$LastChangedRevision: 1027 $".split()[1],
)
...
class TriggerEvent(eg.ActionBase):
    ...
    class text:
        ...
        text2 = "Event payload (optional):"
        text3 = "Delay the firing of the event:"
        text4 = "seconds. (0 = fire immediately)"
        
    
    def __call__(self, eventString, eventPayload="", waitTime=0):
        eventString = eg.ParseString(eventString)
        if eventPayload != '':
        	eventPayload = eg.ParseString(eventPayload)
        else:
        	eventPayload = None
        if not waitTime:
            eg.TriggerEvent(eventString, eventPayload)
        else:
            eg.scheduler.AddTask(waitTime, eg.TriggerEvent, eventString, eventPayload)
    ...
    def Configure(self, eventString="", eventPayload="", waitTime=0):
        panel = eg.ConfigPanel()
        text = self.text
        ...
        sizer1 = wx.FlexGridSizer(cols=2, vgap=5)
        sizer1.Add(panel.StaticText(text.text1), 0, wx.ALIGN_RIGHT, 5)
        sizer1.Add(eventStringCtrl, 0, wx.LEFT, 5)
        sizer1.Add(panel.StaticText(text.text2), 0, wx.ALIGN_RIGHT, 5)
        sizer1.Add(eventPayloadCtrl, 0, wx.LEFT, 5)
