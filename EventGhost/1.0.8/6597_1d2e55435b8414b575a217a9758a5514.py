...
class EventGhost(eg.PluginBase):
    def __init__(self):
        ...
        self.AddAction(Wait)
        self.AddAction(WaitForEventEnd)
...
class WaitForEventEnd(eg.ActionBase):
    name = "Wait for event to end"
    description = "Pauses execution until the event ends. (usefull if something should be triggered at the end of an enduring event, e.g. if you release a button instead of pressing it)"
    iconFile = "icons/Wait"

    class text:
        label = "Wait for the event to end, or %s sec(s)"
        wait = "Timeout:"
        seconds = "seconds"

    def __call__(self, waitTime):
        eg.event.shouldEnd.wait(waitTime)

    def Configure(self, waitTime=10.0):
        panel = eg.ConfigPanel()
        waitTimeCtrl = panel.SpinNumCtrl(waitTime, integerWidth=3)
        panel.AddLine(self.text.wait, waitTimeCtrl, self.text.seconds)
        while panel.Affirmed():
