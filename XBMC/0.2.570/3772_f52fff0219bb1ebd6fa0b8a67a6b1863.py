...
class ActionPrototype(eg.ActionClass):
    def __call__(self):
        try:
            ...
            #self.plugin.xbmc.send_action('xbmc.PlayerControl(Partymode(C:\Users\james\AppData\Roaming\XBMC\userdata\playlists\music\myartist.xsp)', ACTION_EXECBUILTIN)
            #self.plugin.xbmc.send_action('xbmc.PlayerControl(Partymode(C:\myartist.xsp)', ACTION_EXECBUILTIN)
            #self.plugin.xbmc.send_action(self.value, ACTION_BUTTON)
            #self.plugin.xbmc.send_action('xbmc.Container.NextViewMode', ACTION_EXECBUILTIN)
            #---self.plugin.xbmc.send_action('xbmc.PlayMedia(C:\movies.xsp)', ACTION_EXECBUILTIN)
            #self.plugin.xbmc.send_action('xbmc.ActivateWindow(MusicPlaylist)', ACTION_EXECBUILTIN)

            #self.plugin.xbmc.send_mouse_position("1235", "16555")
            #self.plugin.xbmc.send_notification("wel well","icon32x32.png")
            #self.plugin.xbmc.send_notification("Credits","Test")
        except:
            raise self.Exceptions.ProgramNotRunning

# And now we define the actual plugin:
class LoadVoxPlaylist(eg.ActionBase):
    name = "Load voxtemp"
    description = "loads the smartplaylist described by the file c:\\voxtemp.xsp"

    def __call__(self):
        print "trying to load playlist"
        try:
            self.plugin.xbmc.send_action('xbmc.PlayMedia(C:\\voxtemp.xsp)', ACTION_EXECBUILTIN)
        except:
            raise self.Exceptions.ProgramNotRunning
...
class XBMC(eg.PluginClass):
    def __init__(self):
        ...
        group1 = self.AddGroup("My first group","My first group description")
        group1.AddAction(LoadVoxPlaylist)
