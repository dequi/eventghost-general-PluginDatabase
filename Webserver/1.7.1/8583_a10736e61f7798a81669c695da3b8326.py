...
# Enhancement by drhyoung - START
# http://10.1.1.3:81/eg?globals&TMT&NewValue:=78&Another:=My String&[Equation]Lastly:=(1+2+3)&[Flag](3+4+5)
		if event == "globals":
			import urllib 
			queries = remaining.split(u"#", 1)[0].split(u"&") # Repeat to retain UTF-8 chars
			event = queries.pop(0).strip()			  # 
			content = u"<egitems>"
			while len(queries) > 0:
				eginitial = urllib.unquote(queries.pop(0).strip()).decode('utf8')
				egvar = eginitial
				if (egvar[0] == u"[") and (u"]" in egvar):
					eglabel = egvar.split(u"]",1)[0]
					eglabel = eglabel[1:]
					egvar   = egvar.split(u"]",1)[1]
				else:
					eglabel = ""
				if u":=" in egvar:
					egval = egvar.split(u":=",1)[1]
					egvar = egvar.split(u":=",1)[0]
					egvarx = egvar
					if (egvar + u" ")[0].isalpha():
						if egvar[0:11] == u"eg.globals.":
							egvar = egvar[11:]
						egvarx = u"eg.globals." + egvar
				else:
					egvarx = egvar
					if (egvar + u" ")[0].isalpha():
						if egvar[0:11] == u"eg.globals.":
				...
				if eglabel == u"":
					eglabel = egvar
				try:
					if (egvar != egval) and (egval[0] != u"("):
						try:
							float(egval)
						except ValueError:
							egval = u'"' + egval + u'"'
						value = setattr(eg.globals, egvar, eg.ParseString(u"{" + egval + u"}"))
						print "Set  " + egvarx + "  to ", egval
					else:
						egvarx = egval
					content += u'<egitem name="' + eglabel + u'">' + eg.ParseString(u"{" + egvarx + u"}") + u'</egitem>'
				except:
					content += u'<egitem name="' + eglabel + u'"> </egitem>'
					print "globals Error on: ", eginitial
			content += u"</egitems>"
			self.end_request(content, u'text/xml; charset=UTF-8')
			return None
                elif "withoutRelease" in queries:
#               if "withoutRelease" in queries:
# Enhancement by drhyoung - END
