...
class HttpRequestHandler(SimpleHTTPRequestHandler):
    ...
    def SendContent(self, path):
        ...
        if extension not in (".htm", ".html", ".xml"):
            f = self.send_head()
            if f:
	...
	#Handle the post
        print("<h2>Handling Post</h2><P>")
        print("<li>Location: <b>%s</b>"%(self.path))
        print("<li>Arguments: <b>%s</b><hr>"%(self.args))
	self.TriggerEvent(self.path, self.args)
	try:
    ...
    def translate_path_std(self, path): 
