...
# Enhancement by Sem;colon - START
            data=content.split("&")
            if data[0][0:7]=="request":
              self.SendContent(self.path)
              if len(data[0])>7:
                self.plugin.TriggerEvent(data[0][7:], data[1:])
            else:
              self.plugin.TriggerEvent(data[0], data[1:])
              #added some lines from below
              self.send_response(200)
...
# Enhancement by Sem;colon - END
            ...
            if methodName == "GetPersistentValue":   
                ...
                plugin.EndLastEvent()
            #added some spaces in front of these lines
...
class SendEventExt(eg.ActionBase):
    ...
    def __call__(self, event="", host="", user="", password=""):
        ...
        try:
            ...
            try:
                ...
                print "url, username or password is wrong."
        #else:
    ...
    def Configure(self, event="", host="http://127.0.0.1:80", user="", password=""):
        ...
        panel.sizer.AddMany([
            ...
            eventCtrl,
            panel.StaticText("Url: (like you would put it into a webbrowser)"),
            hostCtrl,
