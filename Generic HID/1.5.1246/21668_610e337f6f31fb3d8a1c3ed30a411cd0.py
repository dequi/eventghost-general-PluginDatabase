...
class Text:
    ...
    vendorID = "Vendor ID "
    suspendBox = "Computer Suspend Delay"
    suspendDesc = "Enable this if you are having problems with the plugin functioning\nafter your computer wakes up from suspension.\nYou also can adjust the delay before restart to fine tune it."
    suspendMode = "Enable: "
    wakeWaitPeriod = "Delay: "
...
class HID(eg.PluginClass):
    ...
    def __stop__(self):
        if self.thread: self.thread.AbortThread()
        #unbind from RegisterDeviceNotification message
        try: eg.Unbind("System.DeviceAttached", self.ReconnectDevice)
        except: pass
    ...
    def Configure(self,
        ...
        suspendModeCtrl = panel.CheckBox(suspendMode)
        wakeWaitCtrl = panel.SpinNumCtrl(wakeWait, increment=0.1)
        suspendBox = panel.BoxedGroup(
                                    Text.suspendBox,
                                    Text.suspendDesc,
                                    (Text.suspendMode, suspendModeCtrl),
                                    (Text.wakeWaitPeriod, wakeWaitCtrl)
                                    )

        panel.sizer.AddMany([
                            (optionsSizer, 0, wx.TOP, 10),
                            (suspendBox, 0, wx.TOP, 10)
                            ])
        ...
        while panel.Affirmed():
            ...
            panel.SetResult(
                ...
                suspendModeCtrl.GetValue(),
                wakeWaitCtrl.GetValue()
            )
