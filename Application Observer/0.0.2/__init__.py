...
class Text:
    label_1 = "Observed applications (as notepad.exe):"
...
class ObserverThread(Thread):
    ...
    def __init__(
        ...
        self.apps = apps
        self.running = dict([(app, False) for app in self.apps])
        self.period = period
        self.text = plugin.text
        Thread.__init__(self, name = 'ApplicationObserverThread')
...
class ApplicationObserver(eg.PluginBase):
    ...
    def Configure(self, apps = [], period = 0.5):
        ...
        def OnAddAppCtrl(evt):
            ...
            flag = False
            if len(val) > 4:
#                flag = val[-4:].lower() == ".exe" or val[-4:].lower() == ".com"
                flag = val[-4:].lower() == ".exe"
