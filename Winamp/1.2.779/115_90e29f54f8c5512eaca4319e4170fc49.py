...
eg.RegisterPlugin(
    ...
    version = "1.2." + "$LastChangedRevision: 779 $".split()[1],
    kind = "program",
...
class ActionBase(eg.ActionClass):
    ...
    def SendCommand(self, mesg, wParam, lParam=0):
        ...
        """
        try:
            hWinamp = FindWindow(self.plugin.targetclass)
...
class Winamp(eg.PluginClass):
    ...
    class text:
        ...
        targetClassText = "Target class:"
        targetClassDefault = "(default \"Winamp v1.x\")"
        targetClassDescription = """Set the class of the target Winamp we are trying to control.\n\
To set the Class attribute for a Winamp instance launch it with \"winamp.exe /class=<class>\".\n\
\n\
Leave as default or empty if you don't use multiple instances of Winamp"""
    ...
    def __init__(self):
        ...
        self.AddAction(SetClass)
        self.targetclass = 'Winamp v1.x'
    ...
    def __start__(self, targetclass='Winamp v1.x'):
        self.targetclass = targetclass
        
    def Configure(self, targetclass='Winamp v1.x'):
        panel = eg.ConfigPanel()
        textCtrl = wx.TextCtrl(panel, -1, size=(200, -1))
        textCtrl.SetValue(targetclass)
        #textLabel = wx.StaticText(panel, -1, self.text.targetClassDescription)
        panel.AddLine(self.text.targetClassText, textCtrl, self.text.targetClassDefault)
        panel.AddLine(self.text.targetClassDescription)
        while panel.Affirmed():
            if textCtrl.GetValue() == '':
                panel.SetResult('Winamp v1.x')
            else:
...
class GetPlayingSongTitle(ActionBase):
    ...
    def __call__(self):         #-- v2.0
        try:
            hWnd = FindWindow(self.plugin.targetclass)
        except:
            raise self.Exceptions.ProgramNotRunning
...
class SetClass(ActionBase):
    name = "Set target Winamp class"
    description = "Change the class of the target Winamp we are trying to control."
      
    def __call__(self, targetclass):
        self.plugin.targetclass = targetclass
        return True
    ...
    def Configure(self, targetclass='Winamp v1.x'):
        ...
        textCtrl = wx.TextCtrl(panel, -1, size=(200, -1))
        textCtrl.SetValue(targetclass)
        panel.AddLine(self.plugin.text.targetClassText, textCtrl, self.plugin.text.targetClassDefault)
        panel.AddLine(self.plugin.text.targetClassDescription)
        while panel.Affirmed():
            if textCtrl.GetValue() == '':
                panel.SetResult('Winamp v1.x')
            else:
