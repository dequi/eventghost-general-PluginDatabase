...
class DimGradually(eg.ActionClass):
    ...
    def __call__(self, deviceName, timeInBetween, step, level):
        self.id = self.plugin.getId(deviceName.encode('utf-8'), self.method)
    ...
    def DimGraduallyThread(self):
        ...
        try:
            the_level = int(self.plugin.dimGradually[m_key])
        except:
            pass           
...
class DimPercentage(eg.ActionClass):
    ...
    description = "Dims a TellStick device up or down with a defined step size in percentage."
    iconFile = "lamp-dim"
    method = TELLSTICK_DIM
