...
# Revision history:
#
# 2015-04-01  [Filandre] Added an action to get the state on/off of devices.
...
#            MessageBox(0, self.text.restart_eg, self.text.restart_title, 0)
    ...
    def deviceEventCallback(self, deviceId, method, p1, i3, p2):
        level = 0
        if self.dll.tdLastSentCommand(deviceId, method) == TELLSTICK_DIM:
    ...
    def sensorEventCallback(
        ...
        if strProtocol <> 'oregon' and strProtocol <> 'mandolyn':
            otherEventStr = (
                strProtocol+'|'+
                strModel+'|'+
                str(id)+'|'+
...
class State(DeviceBase, eg.ActionClass):
    name = "State"
    description = "Get the state on/off of a TellStick device."
    iconFile = "lamp-on"
    method = TELLSTICK_TURNON
    ...
    def __call__(self, deviceName):
        ...
        id = self.plugin.getId(self.deviceName, self.method)
        lst_cmd = self.plugin.dll.tdLastSentCommand(id, self.method)
        lst_lvl = self.plugin.dll.tdLastSentValue(id)
        if c_char_p(lst_lvl).value.isdigit():
        	level = int(round(100.0*float((c_char_p(lst_lvl)).value)/255.0, 0))
	else:
	        level = 0
	#print self.deviceName, lst_cmd, lst_lvl
	if level == 0:
		return (lst_cmd & TELLSTICK_TURNON)
	else:
		return level
    ...
    def DimGraduallyThread(self):
        ...
        while not self.finished.isSet():
            ...
            if the_level == int(self.level):
                self.plugin.sendCommand(self.id, 'Dim', the_level)
                k_msg = (
                    str(the_level)
                )
...
class GoodMorning(eg.ActionClass):
    ...
    def __call__(self, deviceName, timeToWakeUp, maxWakeUpLevel):
        ...
        self.maxLevel = int(maxWakeUpLevel*256/100)
    ...
    def GoodMorningThread(self):
        while not self.finished.isSet():
            ...
            if self.plugin.gMschedules:
                self.plugin.sendCommand(self.id, 'Dim', self.maxLevel)
...
#        self.level = int(level)
    ...
    def DimPercentage(self):
        ...
        if self.bStepUp: 
            ...
            if the_level > 255:
                to_send = 255
        self.plugin.sendCommand(self.id, 'Dim', to_send)
