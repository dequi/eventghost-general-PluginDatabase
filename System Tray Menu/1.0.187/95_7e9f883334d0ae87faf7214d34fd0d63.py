...
class Text:
    labelHeader = "Label"
    eventHeader = "Event"
    condHeader = "Condition"                                               #Pako
    editLabel = "Label:"
    editEvent = "Event:"
    editCond = "Condition:"                                                #Pako
    addBox = "Append:"
...
#        self.SetColumnWidth(1, w - self.GetColumnWidth(0))
        self.SetColumnWidth(2, w - self.GetColumnWidth(0)- self.GetColumnWidth(1)) #Pako
        self.__inSizing = False
    ...
    def CopyItem(self, item, parent, prev=None):
        ...
        self.SetItemText(id, self.GetItemText(item, 2), 2)                 #Pako
        return id
...
#        for name, kind, data in reversed(menuData):
        for name, kind, data, cond in reversed(menuData):                  #Pako
            if kind == "item":
                id = wx.NewId()
...
#        for name, kind, data in menuData:
        ...
        def OnAddItem(event):
            ...
            tree.SetItemText(item, "True", 2)                              #Pako
		...
		# Condition edit box                                                   #Pako
        condBox = wx.TextCtrl(dialog, -1)                                  #Pako
        def OnCondTextChange(event):                                       #Pako
            item = tree.GetSelection()                                     #Pako
            tree.SetItemText(item, condBox.GetValue(), 2)                  #Pako
        condBox.Bind(wx.EVT_TEXT, OnCondTextChange)                        #Pako
        ...
        staticText3 = wx.StaticText(dialog, -1, text.editCond)             #Pako
        editSizer.Add(staticText3, 0, wx.ALIGN_CENTER_VERTICAL)            #Pako
        editSizer.Add(condBox, 0, wx.EXPAND)                               #Pako        
        ...
        if dialog.AffirmedShowModal():
            ...
            def Traverse(item):
                ...
                while child.IsOk():
                    ...
                    cond = tree.GetItemText(child, 2)                      #Pako
#                    resultList.append((name, kind, data))                 #Pako
                    resultList.append((name, kind, data, cond))            #Pako
...
class ReinitMenu(eg.ActionClass):                                          #Pako
    name = "Reinit Menu"                                                   #Pako
    description = "Reinit Menu"                                            #Pako
    
    def __call__(self):                                                    #Pako
