...
# -------------------------------------------
# 0.3b by Pako 2014-04-2+ 07:20 UTC+1
#     - mimetype >>> '.json': 'application/json' <<< added
# 0.3a by Pako 2014-04-20 17:27 UTC+1
...
from os.path import getmtime, split, splitdrive, join, isfile
from wx.lib.mixins.listctrl import TextEditMixin
from jinja2 import BaseLoader, Environment, TemplateNotFound
from copy import deepcopy as cpy
from sys import path as syspath
mod_pth = os.path.abspath(split(__file__)[0])
...
class SendEventExt(eg.ActionBase):
    ...
    def Configure(self, event="", host="http://127.0.0.1:80", user="", password=""):
        ...
        userCtrl = panel.TextCtrl(user)
        passwordCtrl = panel.TextCtrl(password)
        hostLabel.SetToolTipString(text.urlTT)
        hostCtrl.SetToolTipString(text.urlTT)
        fl = wx.EXPAND|wx.TOP
        box=wx.GridBagSizer(2, 5)
        box.AddGrowableCol(1)
        box.Add(panel.StaticText(text.event), (0, 0), flag = wx.TOP, border=12)
...
"insecure protocol (http). "
    ...
    def Configure(
        ...
        defDocCtrl.SetToolTipString(text.defDocTool)
        ...
        labels[11].SetToolTipString(text.sslTool)
        labels[12].SetToolTipString(text.sslTool)
        labels[13].SetToolTipString(text.defDocTool)
