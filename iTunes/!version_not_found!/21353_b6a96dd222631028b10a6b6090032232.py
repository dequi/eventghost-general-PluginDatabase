...
class SetPosition(eg.ActionClass):
    ...
    def __call__(self, playerPos):
        if self.plugin.ComActive():
            try:
                self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.SetProperty,self.value,playerPos))
            except:
                #This should mean that the COM server was active, then shutdown and restarted.  
                #The COM instance is from the old thread, so restart
                print "set position restarting thread"
                self.plugin.workerThread.Stop(1)
                self.plugin.workerThread = None
                if self.plugin.ComActive():
                    self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.SetProperty,self.value,playerPos))
...
class SetRating(eg.ActionClass):
    ...
    def __call__(self, rating):
        if self.plugin.ComActive():
            try:
                self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.SetTrackProperty,self.value,rating))
            except:
                #This should mean that the COM server was active, then shutdown and restarted.  
                #The COM instance is from the old thread, so restart
                print "set rating restarting thread"
                self.plugin.workerThread.Stop(1)
                self.plugin.workerThread = None
                if self.plugin.ComActive():
                    self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.SetTrackProperty,self.value,rating))
...
class GetInfo(eg.ActionClass):
    def __call__(self):
        if self.plugin.ComActive():
            try:
                #print "I'm trying"				
                return self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.GetPlayerProperty,self.value))
                ...
                self.plugin.workerThread = None
                if self.plugin.ComActive():
                    print self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.GetPlayerProperty,self.value))
...
class GetURLInfo(eg.ActionClass):
    def __call__(self):
        if self.plugin.ComActive():
            try:
                print "I'm trying"				
                return self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.GetURLProperty,self.value))
            except:
                #This should mean that the COM server was active, then shutdown and restarted.  
                #The COM instance is from the old thread, so restart
                self.plugin.workerThread.Stop(1)
                self.plugin.workerThread = None
                if self.plugin.ComActive():
                    self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.GetURLProperty,self.value))
...
class iTunesEvents():
    ...
    def OnPlayerPlayingTrackChangedEvent(self, iTrack):
        self.plugin.TriggerEvent("UserUpdatedInfo")
...
class iTunesThreadWorker(eg.ThreadWorker):
    ...
    def StdCall(self,value):
        ...
        try:
            ...
            eg.TriggerEvent("Nothing Playing")
    ...
    def doSearchAndPlay(self,searchType,strSearch,Shuffle, PlaylistName):
        ...
        if strSearch != "":
            ...
            if results == None:
                eg.TriggerEvent("Nothing Found")    
            else:            
                for track in results:
                    thePlaylist.AddTrack(track)
                thePlaylist.Shuffle=Shuffle
                thePlaylist.PlayFirstTrack()
                eg.TriggerEvent("iTunesSearchFound")
                print results
        else:
            eg.PrintNotice("iTunesSearchNegative")
    ...
    def ComActive(self):
        ...
        if len(hwnds) != 0:
            if not self.workerThread:
                eg.TriggerEvent("iTunesRunning")
                self.StartThread()
            return True
        elif self.workerThread:
            eg.TriggerEvent("iTunesNotRunning")
...
(ChangeVolume, 'ChangeVolume', 'Change Volume Level', 'Modifies the volume by a percentage (%).', ''),
