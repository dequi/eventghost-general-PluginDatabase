...
class LoadPlaylist(eg.ActionClass):
    ...
    def __call__(self, plname):
        if self.plugin.ComActive():
            self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.doLoadPlaylist,plname))
    ...
    def Configure(self, plname=""):
            ...
            SizerAdd(wx.StaticText(panel, -1, "Playlist Name (case sensitive)"))
...
class SearchAndPlay(eg.ActionClass):
    ...
    def __call__(self, searchType,strSearch, shuffle):
        if self.plugin.ComActive():
            try:
                self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.doSearchAndPlay,searchType,strSearch,shuffle))
            except:
                #This should mean that the COM server was active, then shutdown and restarted.  
                #The COM instance is from the old thread, so restart
                print "set volume restarting thread"
    ...
    def GetLabel(self, searchType,strSearch, shuffle):
        return "Search ("+searchType+"): "+strSearch

    def Configure(self, searchType="",searchString="", shuffle=False):
        panel = eg.ConfigPanel(self)
...
class GetPlaylistTracks(eg.ActionClass):
    class text:
        label_tree="Load Playlist: "
        label_conf="Playlist Name (case sensitive?):"
    def __call__(self, plname):
        if self.plugin.ComActive():
            return self.plugin.workerThread.CallWait(partial(self.plugin.workerThread.GetPlaylistTracks,plname))
...
class iTunesThreadWorker(eg.ThreadWorker):
    ...
    def doLoadPlaylist(self,PlaylistName):
        iTunes = self.comInstance
        try:
    ...
    def GetPlaylistTracks(self,PlaylistName):
        ...
        try:
            ...
            eg.PrintError("error getting playlist tracks")

    def doSearchAndPlay(self,searchType,strSearch,Shuffle):
        iTunes = self.comInstance
        ...
        newPlaylist = iTunes.CreatePlaylist("'Remote")
    ...
    def SimpleActions(self,name):
        ...
        try:
            ...
            eg.PrintError("err: Nothing Playing")
    ...
    def ToggleAction(self,name):
        ...
        if name=="Shuffle":
          #print "I'll try to toggle shuffle!"
          iTunes.CurrentPlaylist.Shuffle = not iTunes.CurrentPlaylist.Shuffle
        else:
          try:
...
(LoadPlaylist, 'LoadPlaylist', 'Load Playlist By Name', 'Loads a playlist by name and plays it.', 'not used'),
...
(GetPlaylistTracks, 'GetPlaylistTracks', 'Get Playlist Tracks By Name', 'Loads a playlist by name and return its tracks', 'not used'),
)
