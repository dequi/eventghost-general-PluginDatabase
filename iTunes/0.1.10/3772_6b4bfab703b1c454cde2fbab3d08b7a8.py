...
class iTunesThreadWorker(eg.ThreadWorker):
    ...
    def doSearchAndPlay(self,searchType,strSearch,Shuffle):
        ...
        try:
            for pl in iTunes.LibrarySource.Playlists:
                if pl.Name == "eventGhostTemp":
                    pl.Delete() 
        except:
            eg.PrintError("error clearing playlist")
        #creating temp playlist
        newPlaylist = iTunes.CreatePlaylist("eventGhostTemp")
...
class iTunes(eg.PluginClass):
    workerThread = None
    def __init__(self):
        self.windowMatch = eg.WindowMatcher(u"iTunes.exe",u'iTunes',u'iTunes',None, None, 1, False, 0.0, 0)
