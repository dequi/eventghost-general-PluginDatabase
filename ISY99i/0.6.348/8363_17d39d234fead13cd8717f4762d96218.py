...
class ISYeventer(asynchat.async_chat):
	...
	if (data.find("<Event seqnum") > -1):
		...
		#if econtrol=="ST" or econtrol=="RR" or econtrol=="OL":
...
class ISYEvent(eg.PluginClass):
	...
	def SetDevice(self, control, action, flag, node):
		    ...
		    sock.send("POST /services HTTP/1.1\r\n")
		    sock.send("Authorization: Basic "+ base64.encodestring('%s:%s' % (self.plugin.username, self.plugin.password)))       
	...
	def Configure(self, control="", action="", flag="", node=""):
		...
		controlCtrl = wx.Choice(panel, -1, choices = ["DON","DOF","DIM"])
        	controlCtrl.SetStringSelection(control)
       		actionCtrl =wx.Slider(panel, -1, str(action), 0, 255, (10,10),
                size=(150, -1),
	...
	def Configure(self, control="", action="", flag="", node=""):
		...
		st2 = panel.StaticText("Action:")
	        ...
	        box1 = panel.BoxedGroup("Attributes", (st1, controlCtrl), (st2, actionCtrl), (st3, flagCtrl), (st4, nodeCtrl) )
	        panel.sizer.AddMany([
	            (box1, 0, wx.EXPAND)
	        ])
	...
	def SetLogLevelCommand(self, level):
		    ...
		    authstr="<s:Envelope><s:Body><u:SetDebugLevel xmlns:u=\"urn:udi-com:service:X_Insteon_Lighting_service:1\"><option>"+str(level)+"</option></u:SetDebugLevel></s:Body></s:Envelope>"
			...
			self.PrintError(answer.strip())
	                sock.close()
	                return False
	            self.plugin.TriggerEvent("Set Log Level to "+level, str(self.plugin.host)+":"+str(self.plugin.port))
		    sock.close()
		    return True
