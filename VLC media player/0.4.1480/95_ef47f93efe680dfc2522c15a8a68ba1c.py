...
#    version = "0.4." + "$LastChangedRevision$".split()[1],
    version = "0.4." + "$LastChangedRevision 1480 $".split()[1],
    kind = "program",
...
def GetVlcVersion():
    """
    Tries to get the VLC version through querying the Windows registry.
    """
    ver = "0.0.0"
    try:
        vlc = _winreg.OpenKey(
        ...
        try:
            ver = _winreg.QueryValueEx(vlc, "Version")[0]
        except:
            pass
        _winreg.CloseKey(vlc)
    except:
        pass
    return ver
...
class Start(eg.ActionBase):
    ...
    def GetCmdLineArgs(self, cmdLineArgs):
        args = '--extraintf=%s --rc-host=%s:%d --rc-quiet --rc-show-pos' % (
            ("oldrc", "rc")[int(GetVlcVersion() < "1.1.0")],
            self.plugin.host, 
            self.plugin.port
        )
        if cmdLineArgs:
            args = "%s %s" % (args, cmdLineArgs)
        return args


    def GetLabel(self, cmdLineArgs=""):
        if cmdLineArgs:
            return "%s: %s" % (self.name, cmdLineArgs)
        else:
            return self.name
...
class VLC(eg.PluginBase):
    ...
    def Configure(self, host="localhost", port=1234, showFeedbackEvents=True): 
        ...
        panel.sizer.Add(eventBox, 0, wx.TOP|wx.EXPAND, 15)
        verSizer = wx.BoxSizer(wx.HORIZONTAL)
        verSizer.Add(wx.StaticText(panel, -1, text.ver), 0, wx.LEFT, 3)
        verText =  wx.StaticText(panel, -1, GetVlcVersion())
        verText.SetForegroundColour(wx.RED)
        verSizer.Add(verText, 0, wx.LEFT, 8)
        panel.sizer.Add(verSizer, 0, wx.TOP|wx.EXPAND, 15)
        while panel.Affirmed():
            panel.SetResult(
                hostCtrl.GetValue(), 
                portCtrl.GetValue(), 
