...
class AutoRemotePayload:
    ...
    def GetDescription(self):
        ...
        if doAllLogs:
            result = ""
            if self.messageObj.communication_base_params['fallback']:
    ...
    def do_GET(self):
        ...
        try:           
            ...
            if result is not None:
                ...
                try:
                    message = urlparse.parse_qs(parsed.query)['message'][0]
...
class GoogleDrive(object):
    ...
    def __init__(self, plugin, refreshToken):
        self.plugin = plugin
...
def replacePythonCodeAndEncode(text):
    try:
        list = pythonSubstitutionPattern.findall(text)
        toEval = [item.replace('{','').replace('}','') for item in list ]
        for item in toEval:
            text = text.replace('{' + item + '}', eval(item))
...
class Communication(object):    
    ...
    def SendSync(self, egClass):
        ...
        if device is not None:
            ...
            if isLocalRequest:
                try:
                    url = "http://" + device.localIp + ":" + device.port + "/"
                    ...
                    print "Couldn't make request via local network."
                    isLocalRequest = False
...
class ShowInputDialog(eg.ActionBase):
    ...
    def __call__(self,  dialogTitle="", dialogText="", textBoxText="", eventName="", useTextForEvent=False):
        ...
        class MyDialog():
            def __init__(self):
                dlg = wx.TextEntryDialog(None, dialogText,dialogTitle, textBoxText)
...
class SendNotification(eg.ActionBase):
    ...
    def __call__(self, key="",  name="", title="", text="", url="", channel="", message="", id="", action="", icon="", led="", ledOn="", ledOff="", picture="", share="", action1="", action1name="", action2="", action2name="", action3="", action3name="", sound="", statusbaricon = "", action1icon = "", action2icon = "", action3icon = "", ticker = "", dismissontouch = "", priority = "", number = "", contentInfo = "", subtext = "", maxprogress = "", progress = "", indeterminateprogress = "", actionondismiss = "", cancel = "", ttl = ""):
        ...
        notification.SetMaxprogress(maxprogress).SetProgress(progress).SetIndeterminateprogress(indeterminateprogress).SetActionondismiss(actionondismiss).SetCancel(cancel)
...
class AutoRemote(eg.PluginBase):
    ...
    def __start__(
        ...
        self.googledrive = GoogleDrive(self, googleDriveRefreshToken)
    ...
    def Configure(
        ...
        self.updateCtrl = self.addLine(None,panel.Button("Check for updates"))
