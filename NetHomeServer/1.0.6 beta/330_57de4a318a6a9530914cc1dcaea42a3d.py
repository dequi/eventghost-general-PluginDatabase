...
import telnetlib, socket, time, os, sys, winsound, re, math, calendar, pickle
...
class loggerThread(Thread):
    ...
    def run(self):
        ...
        wdLevels_60 = []
        wdLabel = ""
        wdLabelPrevious = "Unknown"
 
        #Get persistent weather data if it exists
        majorVersion, minorVersion = sys.getwindowsversion()[0:2]
        if majorVersion > 5:
            progData = os.environ['ALLUSERSPROFILE']
            if (
                not os.path.exists(progData+'/EventGhost/NetHomeServer_data')
                and not os.path.isdir(progData+'/EventGhost/NetHomeServer_data')
            ):
                os.makedirs(progData+'/EventGhost/NetHomeServer_data')
            else:
                print self.name, self.text.txt_ReadingData
                try:
                    f = open (
                        progData
                        +'/EventGhost/NetHomeServer_data/rain_Week_levels_'
                        +str(self.name), 'r'
                    )
                    self.rain_Week_levels = pickle.load(f)
                    f.close()
                except IOError:
                    self.rain_Week_levels = [0.0]*7
                ...
                try:
                    f = open (
                        progData
                        +'/EventGhost/NetHomeServer_data/rain_Week_dates_'
                        +str(self.name), 'r'
                    )
                    self.rain_Week_dates = pickle.load(f)
                    f.close()
                except IOError:
                    self.rain_Week_dates = [0.0]*7
        else:
            if (
                not os.path.exists('NetHomeServer_data')
                and not os.path.isdir('NetHomeServer_data')
            ):
                os.mkdir('NetHomeServer_data')
            else:
                print self.name, self.text.txt_ReadingData
                try:
                    f = open (
                        'NetHomeServer_data/rain_Week_levels_'
                        +str(self.name), 'r'
                    )
                    self.rain_Week_levels = pickle.load(f)
                    f.close()
                ...
                try:
                    f = open (
                        'NetHomeServer_data/rain_Week_dates_'
                        +str(self.name), 'r'
                    )
                    self.rain_Week_dates = pickle.load(f)
                    f.close()
                        ...
                        if (
                            ...
                            if int(upmHouse) != 10:
                                ...
                                if int(upmDevice) == 2:
                                    ...
                                    if len(wdLevels_15) < 15:
                                        wdLevels_15.append(wdSpeed)
                                    else:
                                        wdLevels_15[wi] = wdSpeed

                                    if len(wdLevels_60) < 60:
                                        wdLevels_60.append(wdSpeed)
                                    else:
                                        wdLevels_60[wj] = wdSpeed
                                    
                                    wi += 1
                                    wj += 1
                                    
                                    for k in range(len(wdLevels_15)):
                                        windSpeedSum_15 += wdLevels_15[k] 
                                    wdSpeedAv_15 = windSpeedSum_15/wj

                                    for m in range(len(wdLevels_60)):
                                        windSpeedSum_60 += wdLevels_60[m] 
                                    wdSpeedAv_60 = windSpeedSum_60/wj
                                ...
                                if int(upmDevice) == 3:
                                    ...
                                    if (
                                        rain_Hour[1] == 0.0
                                        or rainLevel < rain_Hour[0] # Rain gauge resetted
                                    ):
                                        rain_Hour[0] = rainLevel
                                        rain_Today[0] = rainLevel
                                        rain_Hour[2] = 0.0
                                        rain_Today[1] = 0.0
                                        rain_Hour_previous = 0.0
                                        start = time.clock()
                                        rain_Hour[1] = start
                                        rain_Hour[3] = 0.0
                                        td_diff_level = rainLevel
                                        
                                    if rainLevel > rain_Hour[0]:
                                        now = time.clock()
                                        rain_Hour[2] = rainLevel
                                        rain_Today[1] = rainLevel
                                        rain_Hour[3] = now

                                        #Last hour
                                        h_diff_level = rain_Hour[2] - rain_Hour[0]
                                        h_diff_time = rain_Hour[3] - rain_Hour[1]
                                        if h_diff_time > 3600*60 :
                                            rain_Hour[0] = rain_Hour[2]
                                            rain_Hour[1] = rain_Hour[3]
                                            rain_Hour_previous = h_diff_level
                                            
                                        #Last day & yesterday & week
                                        td_diff_level = rain_Today[1] - rain_Today[0]
                                        t_now = time.strftime("%H", time.localtime())
                                        if int(t_now) == 0 and not bNewDay :
                                            bNewday = True # A new day begins
                                            rain_Today[0] = rain_Today[1]
                                            self.rain_Week_levels[dy] = td_diff_level
                                            self.rain_Week_dates[dy] = yesterDate
                                            td_diff_level = 0.0
                                        if int(t_now) > 0 and bNewday :
                                            bNewday = False
                                                                      
                                    for item in rain_Week_levels:
                                        rain_Week_sum += item
                                    rain_Yesterday = self.rain_Week_levels[dy]
    ...
    def Abortlogger(self):
        majorVersion, minorVersion = sys.getwindowsversion()[0:2]
        #Make current weather data persistent
        if majorVersion > 5:
            progData = os.environ['ALLUSERSPROFILE']
            if(
                not os.path.exists(progData+'/EventGhost/NetHomeServer_data')
            ...
            pickle.dump(self.rain_Week_levels, f)
            f.close()
            f = open (
                progData
                +'/EventGhost/NetHomeServer_data/rain_Week_dates_'
            ...
            pickle.dump(self.rain_Week_dates, f)
            f.close()
        else:
            if(
                not os.path.exists('NetHomeServer_data')
