...
import socket, time, os, sys, winsound, re, math, calendar, pickle, random
        ...
        while (self.abort == False):
            ...
            if lst.count("NetHomeServer") > 0:
                if not self.bTaskAdded or lst != self.old_lst:
                    ...
                    # Code used for Rain and Wind gauge simulation ####                
                    if bSimulate:
                        iRainSim += 1
...
class NetHomeServer(eg.PluginClass):
    ...
    def main(self,mainThreadEvent):
        ...
        try:
            ...
            if rsp.find("ok") != -1:
                #print self.text.connection_etablished, self.skt
        ...
        while not mainThreadEvent.isSet():
            if self.semaPhore:
                ...
                try:
                    ...
                    #print self.text.connection_error, "First detection", e
                    connectionError = True
