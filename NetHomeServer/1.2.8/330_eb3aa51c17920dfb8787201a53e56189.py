...
# Revision history:
#
# 2016-07-28  Improved handling of socket connection problems
# 2016-02-04  Bugfix for sensors (fineoffset)
# 2015-09-19  Routines for monitoring of controller status slighly improved
# 2015-09-07  Further improved routines for monitoring of controller status
#             (i.e TellStick and others)
# 2015-08-12  Monitoring of controller (i.e TellStick and others) disconnects
# 2015-06-21  Changed a couple of print messages to regular events
# 2015-05-28  Added a connection monitor to report connection problems with
#             OpenNetHomeServer
# 2015-03-31  Bugfix: lost sensors was reported as lost even if they where
#             found back (non-critical bug but anyway...)
#             Added support for more 'Lamps', both events and commands
#             Optimized parts of the code
# 2015-03-15  Added support for RollerTrol events and commands
#             Added support for more 'Lamps', both events and commands
#             Optimized parts of the code
...
class Text:
    ...
    conn_lost = "Connection with NetHomeServer lost"
                    ...
                    if len(lst) > 0 or iPronto > 0:
                        ...
                        if (s_lst.find('Pronto.Message') > -1
                            ...
                            for item in range (7, len(lst)-4):
                                ...
                                if item < len(lst):
                                    msg += "|"
                            eg.TriggerEvent(msg, prefix = self.name)
...
#            self.RestartAllLoggers()
    ...
    def connMonitor(self, monitored, timeout):
        try:
    ...
    def connLost(self, myArgument):
        if not self.conn_error:
            eg.TriggerEvent(myArgument, prefix = self.prefix)
            self.conn_error = True
    ...
    def main(self,mainThreadEvent):
        ...
        def reconnectToHost():
            print 'socket.error, reconnect to host'
            try:
            ...
            if self.semaPhore:
                ...
                try:
                    if cCount > 10:
                        ...
                        self.connectionError = False
                        try:
                            eg.scheduler.CancelTask(self.tmr)
                            self.tmr = None
                    ...
                    if self.tmr == None:
                        print 'Schedule a reconnection'
                        #self.tmr = eg.scheduler.AddTask(self.socketTout, reconnectToHost)
                        self.tmr = eg.scheduler.AddTask(self.rcTimer, reconnectToHost)

                if tst.find('Value,') and event_Old != tst:
                    conn_monitor = self.connMonitor(
                        conn_monitor, 
                        self.socketTout*5
                    )
                    event_Old = tst
...
class RollerTrolCommand(eg.ActionClass):
    ...
    def Configure(
        ...
        # Create a dropdown for object selection
        objCtrl = self.objCtrl = wx.Choice(parent=panel, pos=(10,10)) 
        objCtrl.AppendItems(strings=devices) 
