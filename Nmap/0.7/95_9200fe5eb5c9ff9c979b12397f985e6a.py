...
| Reliability of scanning is not 100%. The fault lies not on the side of Nmap_, 
 ...
 Then this means, that at the default setting of 8 is a disconnection detected 
...
class Nmap(eg.PluginBase):
    ...
    def __start__(
        ...
        self.period = per
        t = int(ttime())
        ct = self.period - t % self.period
        t = t + ct
        self.workerFlag = False
        #self.task = eg.scheduler.AddTask(0.01, self.worker, t - 45)
        self.task = eg.scheduler.AddTask(
            0.01, self.worker, t - int(1.5 * self.period)
        )
        ...
        if self.macs is not None:
            ...
            if added or removed:
                ...
                for gr in self.groups:
                    if len(grState[gr][1]) > len(self.grState[gr][1]):
                        self.TriggerEvent(
                            "Increased.%s" % gr,
                            (gr, grState[gr][1])
                        )
                    elif len(grState[gr][1]) < len(self.grState[gr][1]):
                        self.TriggerEvent(
                            "Decreased.%s" % gr,
                            (gr, grState[gr][1])
                        )
                    if grState[gr][0] != self.grState[gr][0]:
                        self.TriggerEvent(
                            "%s.%s" % (self.text.grState[grState[gr][0]],gr),
                            (gr, grState[gr][0])
                        )
                    if grState[gr][0] != self.grState[gr][0] or \
                        grState[gr][1] != self.grState[gr][1]:
    ...
    def getGroupStates(self):
        ...
        for gr in self.groups:
            ...
            if len(st) == len(membs):
                ...
                state = 3 #Complete
            grState[gr] = (state, list(set(membs).intersection(set(self.macs))))
        return grState
    ...
    def Configure(
        ...
        label_7 = wx.StaticText(panel, -1, self.text.label_3)
        ...
        named_grid = ListCtrl(panel, self.text.header1, named, 5, self)
        self.named_grid = named_grid
        group_grid = GroupListCtrl(panel, self.text.header2, groups, 2, self)
        ...
        Sizer.AddGrowableCol(1)
        Sizer.AddGrowableRow(7, namedSizer.GetSize()[1])
        Sizer.AddGrowableRow(8, groupSizer.GetSize()[1])
        ...
        Sizer.Add(label_7, (3, 0), flag = wx.TOP|ACV)
        Sizer.Add(periodCtrl, (3, 1))
        Sizer.Add(label_3, (4, 0), flag = wx.TOP|ACV)
        Sizer.Add(ipRngCtrl, (4, 1), flag = wx.EXPAND)
        Sizer.Add(label_4, (5, 0), flag = wx.TOP|ACV)
        Sizer.Add(ignoredCtrl, (5, 1), flag = wx.EXPAND)
        Sizer.Add(enbRawCtrl, (6, 0), (1, 2))
        Sizer.Add(namedSizer, (7, 0),(1,2), flag = wx.EXPAND|wx.TOP|ACV)
        Sizer.Add(groupSizer, (8, 0),(1,2), flag = wx.EXPAND|wx.TOP|ACV)
