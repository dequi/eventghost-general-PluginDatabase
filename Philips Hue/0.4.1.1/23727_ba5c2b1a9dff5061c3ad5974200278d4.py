...
class PhilipsHue(eg.PluginBase):
    ...
    def __init__(self):
        ...
        grstatus.AddAction(saveGroupStatus)
        grstatus.AddAction(recallGroupStatus)
...
class saveStatus(eg.ActionBase):
    ...
    def __call__(self, light):
        ...
        if self.plugin.errorLvl('All'): print  res
        return res
...
class saveGroupStatus(eg.ActionBase):
    name = "Save Group status"
    description = "Saves status of Groups. This can be recalled with the recallStatus function."
...
class recallGroupStatus(eg.ActionBase):
    name = "Recall Group status"
    description = "Recalls status of Group saved with saveGroupStatus function."

    def __call__(self, group):
        group = self.plugin.striplight(group)
        dummy=group+10000
        if dummy in self.plugin.saved:
            r = self.plugin.hue(group, self.plugin.saved[dummy],group=True)
            print self.plugin.saved[dummy]
            self.plugin.printError(r)
        else:
            r = False
            if self.plugin.errorLvl('Error'): print 'Error: No saved status for this Group.'
        return r
