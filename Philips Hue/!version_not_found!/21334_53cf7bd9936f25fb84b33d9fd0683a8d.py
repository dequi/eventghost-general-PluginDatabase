...
#
# Copyright (C) 2015-2016  Aquila <post@arneespedal.com>
#
...
# - Changing lights (with this plugin) via groups/scenes will in some cases still trigger an event.
#
# Planed functions:
    ...
    def catchEvents(self, stopThreadEvent):
        ...
        while not stopThreadEvent.isSet():
            ...
            if list != False:
                ...
                for x in range(len(old_all)):

                    #Changed light
                    if old_all[x] != self.all_lights[x]:
                        if self.changed.count(x)==0:
                            if old_all[x]['state']['reachable'] != self.all_lights[x]['state']['reachable']:
                                if old_all[x]['state']['reachable'] == True: self.TriggerEvent(str(x) + ".unreachable")
                                else: self.TriggerEvent(str(x) + ".reachable")
                            if old_all[x]['state']['on'] != self.all_lights[x]['state']['on']:
                                if self.all_lights[x]['state']['on']: self.TriggerEvent(str(x)+ ".on")
                                else: self.TriggerEvent(str(x)+ ".off")
                            else:
                                self.TriggerEvent(str(x)+ ".changed")
            else: 
                wait = 5.0
