...
#
# Copyright (C) 2015  Aquila <post@arneespedal.com>
#
...
#
# Planed functions:
#   - catchEvent will be able to detect changes in settings for hue lights and groups. (optional, setting i config)
#
...
class PhilipsHue(eg.PluginBase):
    ...
    def __start__(self, bridge,username, devicename='My Computer', outlvl='Errors only'):
    ...
    def Configure(self, bridge="", username="EventGhost", devicename='My Computer', outlvl='Errors only'):
        ...
        while panel.Affirmed():
            ...
            panel.SetResult(bridgeHostEdit.GetValue(), usernameEdit.GetValue(), devicenameEdit.GetValue(), selectLvl.GetStringSelection())
...
class Pulse_30sec(eg.ActionBase):
    name = "Pulse ligth for 30 seconds"
...
class ChangeLight(eg.ActionBase):
    ...
    def __call__(self, light, brightness, saturation, color, ttime, alert=0, effect=0):
        command = {'on' : True}
        if brightness != '': 
            try:
    ...
    def Configure(self, light=1, brightness='', saturation='', color='', ttime='', alert=0, effect=0):
        ...
        while panel.Affirmed():
            panel.SetResult(selectLight.GetStringSelection(), brightnessEdit.GetValue(), saturationEdit.GetValue(), colorEdit.GetValue(), ttimeEdit.GetValue(), selectAlert.GetCurrentSelection(), selectEffect.GetCurrentSelection())
...
class saveStatus(eg.ActionBase):
    ...
    def __call__(self, light):
        ...
        if self.plugin.errorLvl('All'): print 'Saved: ' + res
        return res
...
class isGroupOn(eg.ActionBase):
    ...
    def __call__(self, group):
        ...
        if self.plugin.errorLvl('All'): print 'Info: ' + r['action']['on']
        return r['action']['on']
...
class createScene(eg.ActionBase):
    ...
    def __call__(self, list, newname, scene):
