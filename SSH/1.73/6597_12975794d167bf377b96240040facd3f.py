...
class Controller():
    ...
    def __init__(self, ip, port, user, pw, host, comm, plugin):
        self.plugin=plugin
    ...
    def Connect(self, ip, port, user, pw, host):
        if self.connected==False and self.isAlive(ip, port):
            ...
            try:
                ...
                print 'SSH: Faild to connect to '+ip+':'+str(port)
                return False
                ...
                if self.channel.recv_ready():
                    ...
                    if self.commandIsProcessed:
                        ...
                        if self.startLine=="":
                            for i in data:
                                i=i.strip()
                                if len(i)>0 and (i[-1]=="$" or (i[-1]=="#" and i[0]!="#")):
                                    end=i.find(":")
                                    if  end!=-1:
                                        userend=i.find("@")
                                        if userend!=-1:
                            ...
                            if data[-1].find(self.startLine)!=-1:
                                ...
                                if self.onlyIfChanged==False or self.responseList[self.lastCommands[0]]!=data:
                                    ...
                                    if self.addCommand:
                                        self.plugin.TriggerEvent("Data."+self.host+"."+self.lastCommands[0],data)
                                    else:
                                        self.plugin.TriggerEvent("Data."+self.host,data)
                                self.ReceiveOK()
                    else:
                    ...
                    self.checkcounter+=1
                    if self.checkcounter>=100:
                        self.checkcounter=0
                        if self.isAlive(self.ip, self.port)==False:
                            print "SSH: Timeout reached, device is not responding, will disconnect!"
                            self.Disconnect()
            else:
    ...
    def SendCommand(self, newEvent):
        command, host, addCommand, onlyIfChanged, keepInQ=self.commandQ[0]
...
class AddHost(eg.ActionBase):
    ...
    def __call__(self, ip, port, user, pw, host, comm, replace=False):
        if host in self.plugin.instances:
            ...
            if replace:
                ...
                return False
        Controller(ip, port, user, pw, host, comm, self.plugin)
        return True

    def Configure(self, ip="", port=22, user="", pw="", host="", comm="Log", replace=True):
        text = self.Text
        ...
        eg.EqualizeWidths((st_ip, st_port, st_user, st_pw, st_host, st_comm, st_replace))
...
class RunCommand(eg.ActionBase):
    ...
    def __call__(self, command, host, addCommand=False, onlyIfChanged=False, keep=False):
        if host in self.plugin.instances:
            thisHost=self.plugin.hosts[host]
            thisHost.commandQ.append([command,host,addCommand,onlyIfChanged,keep])
    ...
    def Configure(self, Command="", host="", addCommand=False, onlyIfChanged=False, keep=False):
        panel = eg.ConfigPanel()
        text = self.text
        ...
        while panel.Affirmed():
            panel.SetResult(wx_command.GetValue(),wx_host.GetStringSelection(),wx_addCommand.GetValue(),wx_onlyIfChanged.GetValue(),wx_keep.GetValue())
