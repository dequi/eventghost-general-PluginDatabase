...
import eg
import binascii #Pako

eg.RegisterPlugin(
    name = "Serial Port",
    author = "Bitmonster",
    version = "1.1." + "$LastChangedRevision: 192 $".split()[1],
...
class Serial(eg.RawReceiverPlugin):
    ...
    def HandleChar(self, ch):
        # self.buffer += ch  #Pako
        self.buffer += binascii.b2a_hex(ch) #Pako
    ...
    class Read(eg.ActionClass):
        ...
        def __call__(self, count=None, timeout=0.0):
            ...
            if count is None:
                count = 1024
          #  data = serial.read(count)  Pako
            data = binascii.b2a_hex((serial.read(count))) #Pako
            return data
