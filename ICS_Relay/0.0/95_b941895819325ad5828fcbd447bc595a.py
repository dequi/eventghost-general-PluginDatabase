...
class StatusBitmap(GenStaticBitmap):
    ...
    def OnLeftUp(self, event):
        ...
        if self.currState >= len(self.stateValues):
            self.currState = 0
        self.SetState(self.currState)
        self.SetToolTipString(self.tooltips[self.currState]) 
...
class ICS_Relay(eg.RawReceiverPlugin):
    ...
    def Configure(
        ...
        portSizer = wx.FlexGridSizer(3, 2, 10, 9)
        ...
        buttons = (
            wx.NewId(),wx.NewId(),wx.NewId(),wx.NewId(),
            wx.NewId(),wx.NewId(),wx.NewId(),wx.NewId()
        )
        images = [None, None]
        sbuf = StringIO(b64decode(RED))
        wximg = wx.ImageFromStream(sbuf)
        images[0] = wx.BitmapFromImage(wximg)        
        sbuf = StringIO(b64decode(GREEN))
        wximg = wx.ImageFromStream(sbuf)
        images[1] = wx.BitmapFromImage(wximg)        
...
class SetNewState(eg.ActionBase):
    ...
    def Configure(self, newState = 8 * [0]):
        ...
        wximg = wx.ImageFromStream(sbuf)
        images[2] = wx.BitmapFromImage(wximg)
