...
eg.RegisterPlugin(
    ...
    author = "Me. made original, modified by krambriw, further modified by Ersan",
    version = "0.0.5",
    kind = "external",
...
class HomeseerPlugin(eg.PluginClass):
    ...
    def __start__(self, hostname, username, password):
        ...
        self.hsi.SetHost(self.hostname) # Ersan: fixed remote connections to homeseer
...
#            self.hs =  Dispatch("homeseer.application")
    ...
    def doEvent(self, eventName): # Ersan: add doEvent command
        if self.connected:
            if self.hs.EventExists(eventName) == True: # ersan: Check if the event exists
                print "Running event " + eventName
                self.hs.TriggerEvent(eventName)
            else:
                print eventName + " does not exist in Homeseer configuration."			
        else:
            print "Not connected to Homeseer."		
...
class Event(eg.ActionClass):
	
    def __call__(self, eventName):
        self.plugin.doEvent(eventName)


    def Configure(self, event=""):
        panel = eg.ConfigPanel()
		
        eventTextControl = panel.TextCtrl(event)
        
        sizer = wx.FlexGridSizer(rows=2, cols=2, hgap=10, vgap=5)
        sizer.Add(panel.StaticText("Event Name: "))
        sizer.Add(eventTextControl)  
        ...
        while panel.Affirmed():
            panel.SetResult(eventTextControl.GetValue())		
