...
class Server:
    ...
    def RunListener(self, port, sendaddr, vendor, client, pretype, heartbeat):
        text = self.text
        ...
        while self.hubmode[0]:
            ...
            if not self.UDPEvent.isSet():
                args = [data, inaddr[0], recvaddr, sendaddr, pretype, hubname, clientname]
    ...
    def IncomingData(self, data, inaddr, recvaddr, sendaddr, pretype, hubname, clientname):
        if not data: return
        plg = self.plugin
        ...
        Class = dc(payload['Class']) if 'Class' in payload else text.NoData1
        Target = dc(payload['Target']) if 'Target' in payload else text.NoData1
        Source = dc(payload['Source']) if 'Source' in payload else text.NoData1
        Interval = dc(payload['Interval']) if 'Interval' in payload else '60'
        HeartBeat = dc(payload['Class']) if Type == 'xap-hbeat' else None

        newEvent = dc(plg.CheckEvent([dc(Source), dc(Class), dc(Type), dc(Target)]))
        if not self.Log(">", text.DataLbl+data, newEvent): return
        
        if HeartBeat:
            args = [3639, sendaddr, plg.vendor, self.client, pretype, self.heartbeat]
            if recvaddr == inaddr:
                if self.hubmode[0] == 'xAP-Hub' and Source != hubname:
                    lease = text.TermnLs if HeartBeat.find('alive') == -1 else text.RenewLs
                    try:
                        idx = self.leaseData.index([UID, Port])
                        newLease = self.leaseData.pop(idx)
                        self.leaseData.pop(idx).cancel()
                    except:
                        if HeartBeat.find('alive') > -1:
                            newLease = [UID, Port]
                            lease = text.ForwdLs
        ...
        presuf = [Source]
        presuf += ['xAP'+Class[3:].title() if HeartBeat else Class]
        ...
        try:
            #test = eg.PluginManager()
            #info = test.GetPluginInfo("{0AF02D47-9C43-4D86-A6B2-C5997CC4E9C6}")
        ...
        if clientname and Target and clientname == Target:
            kwargs = dict(prefix=dc(payload['Prefix']) if 'Prefix' in payload else None)
            kwargs['suffix'] = dc(payload['Suffix']) if 'Suffix' in payload else None
            kwargs['payload'] = dc(payload['Payload']) if 'Payload' in payload else None
            if kwargs['suffix'] is None: del kwargs['suffix']
            if kwargs['payload'] is None: del kwargs['payload']
            try:
                if kwargs['payload'][:5] == "'{eg." and kwargs['payload'][-2:] == "}'":
                    kwargs['payload'] = dc(eval(kwargs['payload'][2:-2]))
                else:
                    kwargs['payload'] = dc(eval(kwargs['payload']))
            except:
                pass
            eg.TriggerEvent(**kwargs)
        elif Target and Target.find(hubname+":") > -1:
...
class xAP(eg.PluginBase):
    ...
    def __start__(
        ...
        if hostname in self.GetAddresses():
            self.Server.Start(self.port, hostname, vendor, client,  pretype, heartbeat)
    ...
    def Configure(
        ...
        st15 = wx.Button(panel, -1, text.IgDbBtn)

        def getPanelResults():
            return [addrs[st5.GetValue()], st6.GetValue(), UID, st11.GetValue(),
                        st13.GetValue(), str(st14.GetValue()), st4.GetValue()]
        ...
        panel.sizer.Add(panel.BoxedGroup(text.CltBox, st3, (text.CltText, st4)), 0, wx.EXPAND|wx.ALL, 5)
        panel.sizer.Add(panel.BoxedGroup('', (text.IPAText, st5), (text.HstText, st6)), 0, wx.EXPAND|wx.ALL, 5)
        uidSizer.Add(st7, 0, wx.ALIGN_RIGHT)
        uidSizer.Add(st8, 0, wx.ALIGN_CENTER)
        uidSizer.Add(st9, 0, wx.ALIGN_LEFT)
        panel.sizer.Add(panel.BoxedGroup(text.UIDBox, uidSizer), 0, wx.EXPAND|wx.ALL, 5)
        panel.sizer.Add(panel.BoxedGroup(text.VenBox, st10, (text.VenText, st11)), 0, wx.EXPAND|wx.ALL, 5)
        panel.sizer.Add(panel.BoxedGroup(text.VerBox, st12, (text.VerText, st13)), 0, wx.EXPAND|wx.ALL, 5)
        panel.sizer.Add(panel.BoxedGroup(text.HbtBox, (text.HbtText, st14)), 0, wx.EXPAND|wx.ALL, 5)
        btnSizer.Add(st15, 0, wx.ALIGN_CENTER)
        panel.sizer.Add(panel.BoxedGroup('', btnSizer), 0, wx.EXPAND|wx.ALL, 5)

        st8.Bind(wx.EVT_CHAR, onUpdateUID)
        st13.Bind(wx.EVT_CHOICE, onChoice)
        st15.Bind(wx.EVT_BUTTON, onIgnDbg)
...
class sendxAP(eg.ActionClass):
    ...
    def Configure(self, source='', typ="Audio.Transport", cls="xAP-Audio.Transport",
                                target="ersp.SlimServer.SOMEHOSTNAME:PLAYERNAME", msg="Command=Play"):
