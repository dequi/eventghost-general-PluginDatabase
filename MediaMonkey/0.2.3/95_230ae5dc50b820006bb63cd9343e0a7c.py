...
class MediaMonkeyWorkerThread(eg.ThreadWorker):
    ...
    def LoadPlaylistByTitle(self, plString,repeat,shuffle,crossfade,accessible):
        if self.isRunning():
            ...
            if Total > 0:
                ...
                if not accessible:
                    ...
                    n=0
                    for itm in range(Total):
                        if isfile(plItems.Item(itm).Path):
                            self.MM.Player.PlaylistAddTrack(plItems.Item(itm))
                            n+=1
                    if n > 0:
                        self.MM.Player.Play()
            if self.plugin.trigger:
                if not accessible:
                    ...
                    self.plugin.TriggerEvent(Text.playlistEvt, (plString, str(n), str(Total)))
                    return n,Total
    ...
    def LoadFilterPlaylist(
        ...
        if self.isRunning():
            ...
            if self.plugin.trigger:
                if accessible:
                    self.plugin.TriggerEvent(Text.playlistEvt, (plName, str(n), str(Total)))
                    return n,Total
                else:
...
class Text:
    ...
    triggerEventsToolTip = '''Some actions (such as %s
or %s) may, after its completion,
...
Choose whether EventGhost will wait for the result, or not.'''
    ...
    crossfade = "Crossfade"
    accessible = "Load only accessible tracks (lower speed !)"
...
class LoadPlaylist(eg.ActionBase):
    ...
    def __call__(self, plString, repeat, shuffle, crossfade, accessible, wait):
        if not self.plugin.workerThread:
            self.plugin.workerThread = MediaMonkeyWorkerThread(self.plugin)
            self.plugin.workerThread.Start(100.0)
        args = (
    ...
    def Configure(self, plString="",repeat=2,shuffle=2,crossfade=2, accessible = False, wait = 0):
        panel = eg.ConfigPanel(self)
        text = self.text
        ...
        leftSizer.Add(accessibleChkBoxCtrl,0,wx.TOP,15)
...
class AddCurrentSongToPlaylist(eg.ActionBase):
    ...
    def __call__(self, plString, skip, verbose):
        ...
        if verbose < 2:    
            ...
            return res if verbose == 1 else Text.resAdded[res] % plString
        else:
            self.plugin.workerThread.Call(partial(*args))
            return Text.withoutWaiting            
    ...
    def Configure(self, plString="", skip=False, verbose = 0):
        ...
        radioBox.SetItemToolTip(1, "0 "+self.text.forToolTip+"   "+Text.resAdded[0] % "" +\
            "\n1 "+self.text.forToolTip+"   "+Text.resAdded[1] % "" +\
            "\n2 "+self.text.forToolTip+"   "+Text.resAdded[2] % "")
...
class RemoveCurrentSongFromPlaylist(eg.ActionBase):
    ...
    def __call__(self, plString, skip, now_pl, verbose):
        ...
        if verbose < 2:
            res = self.plugin.workerThread.CallWait(partial(*args),60)
            return res if verbose==1 else Text.resRemoved[res] % plString
        else:
            self.plugin.workerThread.Call(partial(*args))
            return Text.withoutWaiting
    ...
    def Configure(self, plString="", skip=False, now_pl=False, verbose = 0):
        ...
        radioBox.SetItemToolTip(1, "0 "+self.text.forToolTip+"   "+Text.resRemoved[0] % "" +\
            "\n1 "+self.text.forToolTip+"   "+Text.resRemoved[1] % "" +\
            "\n2 "+self.text.forToolTip+"   "+Text.resRemoved[2] % "")
        radioBox.SetSelection(verbose)
        SizerAdd(radioBox, 0, wx.EXPAND|wx.TOP,15)
...
class RemoveCurrentSongFromNowPlaying(eg.ActionBase):
    ...
    def __call__(self, skip, verbose):
        if not self.plugin.workerThread:
            ...
            self.plugin.workerThread.Start(100.0)
        if verbose < 2:
            res = self.plugin.workerThread.CallWait(
                partial(
            ...
            return res if verbose==1 else Text.resNowPlay[res]
        else:
    ...
    def Configure(self, skip=False, verbose=0):
        ...
        radioBox.SetItemToolTip(1, "0 "+self.text.forToolTip+"   "+Text.resNowPlay[0]+\
            "\n1 "+self.text.forToolTip+"   "+Text.resNowPlay[1])
        radioBox.SetSelection(verbose)
        SizerAdd(radioBox, 0, wx.EXPAND|wx.TOP,15)
