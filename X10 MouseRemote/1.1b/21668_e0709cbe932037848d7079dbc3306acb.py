...
class SerialThread(threading.Thread):
    ...
    def __init__(self, prefix, timeout, triggerevent, serial, stopevent, logbytes):
...
class X10Mouse(eg.RawReceiverPlugin):
    ...
    def __start__(self, port, timeout, prefix='X10-Mouse', enduringevent=False, logbytes=False):
        if enduringevent:
    ...
    def Configure(self, port=0, timeout=0.145, prefix='X10-Mouse', enduringevent=False, logbytes=False):
        panel = eg.ConfigPanel()
        prefixCtrl = panel.TextCtrl(prefix)
        portCtrl = panel.SerialPortChoice(port)
        timeoutCtrl = panel.SpinNumCtrl(timeout, min=.001, max=1.000, increment=0.001, fractionWidth=3)
        ...
        panel.AddLine('Timeout:', timeoutCtrl)
        panel.AddLine(
