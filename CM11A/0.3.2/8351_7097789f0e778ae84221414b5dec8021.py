...
class Text:
    ...
    class X10Command:
        ...
        function = "Function:"
        functions = ["All Units Off", "All Lights On", "All Lights Off", "On", "Off", "Dim", "Bright", "Extended Code"]
        dimbright = "Dim/Bright %:"
        dimbrights = ["0", "5", "9", "14", "18", "23", "27", "32", "36", "41", "45", "50", "55", "59", "64", "68", "73", "77", "82", "86", "91", "95", "100"]
        timing = "Timing:"
        timings = ["3.5 sec", "30 sec", "1 min", "5min"]
        level = "Level:"
...
class X10Command(eg.ActionClass):
    ...
    def __call__(self, houseIndex=0, deviceIndex=0, functionIndex=3, dimbrightIndex=0, timingIndex=0, level=0):
        text = self.text
        if level > 100 : level = 100
        if level < 0 : level = 0
        levelCode = int(round(level * 62 / 100))
        ...
        if deviceCode == "":
            ...
            if self.plugin.functionMap[function] == 0x07 :
                header = 0x07
                cmd = self.plugin.houseCodeMap[houseCode] | self.plugin.functionMap[function]
                dc = self.plugin.deviceCodeMap[deviceCode]
                data1 = levelCode + (timingIndex << 6)
                data2 = 0x31
                self.SendReceive(chr(header)+chr(cmd)+chr(dc)+chr(data1)+chr(data2), chr((header + cmd + dc + data1 + data2)&0xFF))
                self.SendReceive(chr(0), chr(0x55))
            else:
                header = 0x06 + (dimbrightIndex << 3)
                cmd = self.plugin.houseCodeMap[houseCode] | self.plugin.functionMap[function]
    ...
    def GetLabel(self, houseIndex, deviceIndex, functionIndex, dimbrightIndex, timingIndex, level):
        text = self.text
        houseCode = text.houseCodes[houseIndex]
        deviceCode = text.deviceCodes[deviceIndex]
        function = text.functions[functionIndex]
        if function == "Extended Code" :
            function += " (" + str(level) + ")"
    ...
    def Configure(self, houseIndex=0, deviceIndex=0, functionIndex=3, dimbrightIndex=0, timingIndex=0, level=0):
        text = self.text
        panel = eg.ConfigPanel(self)
        ...
        timingCtrl = panel.Choice(timingIndex, text.timings)
        levelCtrl = wx.Slider(panel,-1,level, 0, 100)
        
        def OnStateChoice(event):
            dimbrightCtrl.Enable(functionCtrl.GetValue() in (5,6))
            timingCtrl.Enable(functionCtrl.GetValue() == 7)
            levelCtrl.Enable(functionCtrl.GetValue() == 7)
            event.Skip()
        ...
        panel.AddLine(self.text.timing, timingCtrl)
        panel.AddLine(self.text.level, levelCtrl)
